...
| **DEC-U3-3** | **[2026-09-16 개정] 초안 단계도 검증 시각을 표시한다.** 초안·완성·확정 전 단계가 **검증 시각**을 노출한다. 고정 블록(숙소·시각 고정 필수 방문지)은 종전과 동일하게 초안에서도 시각 노출 | Q3=A(원 결정, 2026-08). 초안 시각은 슬롯 교체마다 흔들려 신뢰를 깎는다. 안 보여주는 것은 INV-2 위반이 아니다. → **[TRIP-783, 2026-09-16] Figma h08 재설계(`4221:2448`/`4224:2448`)가 초안 카드에도 전 슬롯 시각 칩을 그리면서 이 결정과 충돌, 3-a에서 "Figma대로 전 슬롯 시각(불변식 갱신)"으로 사용자 재확정(화면 정본 우선, TRIP-811 계열). Q3=A의 원 근거(교체 중 흔들림)는 카드가 `timeLabel`을 받으면 그리는 presentation-only라 정책 판단이 소비처로 이동한 것으로 흡수됐다 |
