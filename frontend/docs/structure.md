...
| `formatPrice` | `entities/stay/lib`(TRIP-807 이관, 원래 `features/stay/model`) | `formatPrice(price?: StayPrice \| null): string` — 최저가 스냅숏 → 카드 금액 문자열(TRIP-180, PBT 5건, 출력 바이트 불변). 옛 자리 `features/stay/model/formatPrice.ts`는 TRIP-810으로 삭제됨 — 소비처는 전부 entities 경로에서 직접 import |
...
| `MapView` | `shared/map` | 공급자 중립 지도(네이버 네이티브, TRIP-861). props `center`·`pins?`·`viewOnly?`·`connectPins?`·`onPinTap?`·`onLoadFailed?`·`onCameraIdle?`·`maxLevel?`. 새 지도가 필요하면 이걸 쓴다 — SDK를 직접 import 하지 마라(`mapBridgeStructure` A-4가 shared/map 밖 SDK import 0을 강제) |
| `CenterPinPicker` | `shared/map` | 중앙 고정 핀 좌표 선택기(TRIP-866) — 지도를 움직여 화면 중앙 핀으로 좌표 확정(`onPick({lat,lng})`). 롱프레스 대체 |
| `Toggle` | `shared/ui` | 켜짐·꺼짐·잠김 3모양 공유 스위치(46×28pt, TRIP-780) — `checked`·`disabled`·`onPress`·`testID`. 인라인 스위치를 새로 만들기 전에 이걸 먼저 쓴다. 소비처 3(l06 위치동의·l05 개인화·l02 알림 설정) |
...
| `formatDateRange` · `formatSectionRange` · `formatTripRange` · `formatConfirmedDateRange` · `formatTripDateRange` · `formatDateRangeWithDow` · `dayOfWeek` · `WEEKDAY_LABELS` | `entities/trip/lib/formatTripPeriod`(TRIP-808 이관, 원래 `features/trip/model/{baseScreen,tripSummary,tripWizardStep1}`·`features/itinerary/model/planState`·`features/record/model/recordsCalendar` 각자 사본) | 기간 6벌 + 요일 1벌, **바이트 그대로 이관**(en dash U+2013·미들닷 U+00B7·공백 유무 출력 보존, 병합·통일 없음). 옛 자리 재수출 shim은 TRIP-810으로 정리됨(순수 shim은 파일 삭제, 부분 shim(planState·baseScreen·tripWizardStep1·ItineraryGlyphs)은 재수출 줄만 제거) |
...
| `presetRange` · `PERIOD_PRESETS` · `COMPANION_OPTIONS` · `PeriodPresetCode` · `CompanionCode` | `features/trip/model/tripWizardStep1` | 프리셋 코드 → 실제 날짜 범위(에포크 일수 계산, `new Date(` 미사용, TRIP-205). 시계 안 읽음(기준일은 인자). **`formatDateRange`·`dayOfWeek`는 TRIP-808로 `entities/trip/lib/formatTripPeriod`로 이관** — `dayOfWeek`만 내부(`presetRange`)에서 써 entities 에서 `import`하고, `formatDateRange` 재수출은 TRIP-810으로 제거됨(위 entities/trip 행 참고) |
...
| `daysInMonth` · `firstWeekdayOfMonth` · `shiftMonth` · `isDateInRange` · `buildMonthGrid` | `shared/date/monthGrid` | **TRIP-639로 stay·trip 두 사본 통합.** 월 그리드 순수 산술(에포크 일수 기준) 정본 — `features/stay/model/stayDates`·`features/trip/model/tripDatePicker`는 더 이상 이 넷을 정의하지 않고 여기서 직접 import한다(재수출 shim 없음). `isDateInRange`는 숙소 판정용이 **아니다** — 세 판이 원래 같은 식이었고 통합 후에도 그렇다(숙소 의미는 `nightsBetween`·`isStayRangeValid`에 있다) |
| `nightsBetween` · `isStayRangeValid` · `applyDatePick` · `commitDateRange` · `formatStayDateRange` · `StayDateRange` | `features/stay/model/stayDates` | 숙소 날짜 판정·전이·표시 순수 함수(TRIP-198, 에포크 일수 기준). **TRIP-639로 그리드 산술 4개(`daysInMonth` 등)는 `shared/date/monthGrid`로 빠졌다** — 이 파일엔 숙소 도메인 판정만 남는다. **여행 기간 계산엔 재사용 불가** — 같은 날/역전을 `null`로 접는 숙소 판정과 여행(0박 합법·역전은 별도 코드)은 의미가 반대다(TRIP-204 01b D5) |
...
| `unresolvedDaysView` · `UnresolvedDaysView` | `features/trip/model/baseScreen` | **신규(TRIP-225)** — g02 미해결 날짜 뷰 순수 함수. 날짜별 상태 → 앞 2개+나머지 개수로 접는 뷰(고르기·자르기·세기 세 관심사 분리). `formatSectionRange`·`formatTripRange`는 TRIP-808로 `entities/trip/lib/formatTripPeriod`로 이관됐고 baseScreen 재수출은 TRIP-810으로 제거됨(위 entities/trip 행 참고) |
...
| `buildSlotKey` · `parseSlotKey` · `buildSlotKeys` · `ParsedSlotKey` · `SlotKeySet` | `entities/itinerary-slot/lib/slotKey`(TRIP-809 이관, 원래 `features/itinerary/model/slotKey`) | slotKey 조립·파싱·목록 충돌 검출(BR-U2-04, 판별 유니온 반환, TRIP-295). **바이트 그대로 이관**(파일째 이사). 옛 자리 `features/itinerary/model/slotKey.ts`는 TRIP-810으로 삭제됨(11개 prod importer 전부 entities 경로로 재조준) |
| `resolveCategoryPlaceholder` · `CategoryPlaceholder` | `entities/itinerary-slot/lib/categoryPlaceholder`(TRIP-809 이관, 원래 `features/itinerary/model/categoryPlaceholder`) | 카테고리→`{tintClass, iconKey}` 순수 매핑(7종+폴백, TRIP-465). **바이트 그대로 이관**. 옛 자리 `features/itinerary/model/categoryPlaceholder.ts`는 TRIP-810으로 삭제됨(소비처 0) |
| `SlotPhotoPlaceholder` | `entities/itinerary-slot/ui`(TRIP-809, 원래 `features/itinerary/ui`) | h25 사진 없는 슬롯 카테고리 틴트+아이콘 플레이스홀더(TRIP-465). `TimelineScreen`이 소비. 옛 자리 `features/itinerary/ui/SlotPhotoPlaceholder.tsx`는 TRIP-810으로 삭제됨(소비처 0) |
| `PoiSlotCard` | `entities/itinerary-slot/ui`(TRIP-809, 원래 `features/itinerary/ui`) | 통일 POI 카드(peek/list, TRIP-301). `TimelineScreen`이 소비. 옛 자리 `features/itinerary/ui/PoiSlotCard.tsx`는 TRIP-810으로 삭제됨(소비처 0) |
| `ReplanSlotRow` · `ReplanSlotVM` · `ReplanSlotTone` | `entities/itinerary-slot/ui`·`model`(TRIP-809, 원래 `features/planb/ui/ReplanSlotRow`) | i06/i13 재계획 슬롯 행 + VM 타입(TRIP-563). `pages/planb-draft/ui/ReplanDraftView.tsx`·`_dev/preview.tsx`(`ReplanSlotVM` 타입)가 소비. 옛 자리 `features/planb/ui/ReplanSlotRow.tsx`는 TRIP-810으로 삭제됨(전부 entities 경로로 재조준). **TRIP-751**: `SlotBadgeKind`→`ReplanSlotTone`으로 이름 교체 + `photo`·`timeLabel`·`categoryLabel`·`distanceRange` 필드 추가(층별 표는 `layer-entities.md` 참고), 소비 화면 `features/planb/ui/{ReplanDraftScreen,NoAlternativeScreen}.tsx`는 파일째 삭제되고 `pages/planb-draft/ui/ReplanDraftView.tsx`(신규)로 흡수됨 |
...
| `withObjectParticle` | `features/itinerary/lib/objectParticle` | **신규(TRIP-800)** — 단어 끝 글자로 목적격 조사 `을`/`를`을 붙인다(한글 음절이 아니면 `을(를)` 병기). 조사를 손으로 붙이는 문구를 또 만들지 말고 이것을 쓴다 |
| `resolvePlanState` · `buildPlanDayTabs` · `isConfirmLocked` · `PlanState` · `PlanDayTab` | `features/itinerary/model/planState` | **신규(TRIP-299) → TRIP-401로 목적지 판정 2함수 추가(별도 행 없음, 같은 파일) → TRIP-337로 `isConfirmLocked` 추가.** h25 순수 판정 3함수 + 판별 유니온 2타입. `ItineraryPlanPage.tsx`가 유일한 소비자. `formatDraftDayHeader`(위 `draftView` 행)는 카드 위 날짜 헤더 표시에 그대로 재사용됐다(같은 feature 내 재사용, 새 포맷터 미신설). **TRIP-337 추가분**: `isConfirmLocked(generationState?): boolean`(1줄) — PARTIAL이면 확정 예방잠금, `resolvePlanState`와 별개 관심사. **`formatNightsLabel`·`formatConfirmedDateRange`는 TRIP-808로 `entities/trip/lib/{formatNights,formatTripPeriod}`로 이관** — planState 재수출은 TRIP-810으로 제거됨, 소비처(ItineraryPlanPage·TripCardContainer)는 entities 경로에서 직접 import(위 entities/trip 행 참고) |
...
| `WheelPicker` · `WheelPickerProps` | `shared/ui/WheelPicker` | **신규(TRIP-599)** — 값-컬럼 휠(스크롤-스냅) 시각 피커 primitive. `values`·`selected`·`onSelect` + `renderLabel`·`testIDForValue` 주입(shared/ui라 도메인 무지 — 소비처가 라벨·testID 접두를 넣는다). `ScrollView`+`snapToInterval` 커스텀(리포에 휠 라이브러리 없음). **선택은 셀 press로 확정**(스크롤 자동확정 아님 — 현행 press→onSelect 계약 보존). h07 `MustVisitTimeScreen` 시작 시각 시트가 첫 소비자. h24 `SlotTimeSheet`·i15/i22 `ManualTimeSheet`(시·분 2컬럼) 이관은 후속. ⚠️ 스냅·중앙정렬·관성은 jest 사각(6-b 실기만) |
...
| `formatRelativeTime` | `shared/date/formatRelativeTime` | **신규(TRIP-576)** — 과거 ISO 시각 → "방금·N분 전·N시간 전·어제·N일 전"(경과시각, INV-3 소요시간과 무관). `shared/date/formatKoreanDate`(절대 `M월 D일 요일`)와는 별 함수 — 재사용 불가 확인 후 신설 |
| `registerPushToken` · `unregisterDeviceToken` · `toServerOsPermission` · `isDeviceNotRegistered` | `shared/push/register` | **신규(TRIP-576)** — Expo 푸시 토큰 등록(`POST /me/push-tokens`)·해제(`DELETE /me/push-tokens/{token}`) 배선 + 로컬 `PushPermissionStatus`(`shared/push/permissions.ts`)→서버 `RegisterPushTokenRequestOsPermission` 매핑(`toServerOsPermission`, `UNDETERMINED→NOT_DETERMINED` — 이름이 비슷한 다른 두 계약, 그대로 넘기면 계약 위반). `isDeviceNotRegistered`는 Expo 영수증 오류 판별(BR-U6-37) |
| `notificationKind` · `NotificationIconKind` | `features/notification/model/notificationKind` | **신규(TRIP-576)** — 서버 `NotificationKind` 8종 → 5 아이콘군·한글 라벨 순수 매핑(`Record` 전수, 일정군 3종이 한 아이콘으로 접힘) |
...
| `useNotificationInbox` | `features/notification/model/useNotificationInbox` | **신규(TRIP-576)** — `useGetMeNotifications`를 얇게 감싼 조회 훅. ⚠️ `isError`를 소비하지 않으면(현재 `NotificationInboxPage`가 그렇다) 조회 실패가 빈 배열로 접혀 "알림 없음"으로 오표시된다(후속 티켓 후보). **`markAllRead`(TRIP-773) 추가** — 미읽음 id마다 `POST /me/notifications/{id}/read`를 `Promise.allSettled`로 순회한 뒤 목록을 무효화하고, 전부 성공했는지 boolean으로 반환한다(`/read-all` 일괄 API 미사용 — 새 티켓 후보, 상세는 `.claude/rules/layer-features-notification.md`) |
...
| `MapSheetShell` · `SheetHeader` · `DistanceConnector` · `CtaBar` · `DayChipOverlay` · `MapFallbackBar` | `widgets/map-sheet-shell/ui` | **신규(TRIP-783, widgets 층)** — 지도+3스냅 시트 셸(닫힘·45%·88%, TRIP-920). 지도 실패 시 셸이 스스로 `MapFallbackBar`로 바꾼다(TRIP-919). 조립 소비처는 결과·편집·여행 중 화면 전반(`DraftPage`·`ItineraryPlanPage`·`CoPickCompletePage`·`PlaceAddPage`·`LiveHubView`·`ReplanSolvingView`·`ReplanDraftView`·`StayRecommendView`·`EditorView`). 새 지도+시트 화면을 또 그리지 말고 이 셸에 `header`·`children`·`cta`를 주입해 쓴다. 상세는 `.claude/rules/layer-widgets.md` |
