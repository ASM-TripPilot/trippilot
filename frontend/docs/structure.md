...
| `ReplanSlotRow` · `ReplanSlotVM` · `SlotBadgeKind` | `entities/itinerary-slot/ui`·`model`(TRIP-809, 원래 `features/planb/ui/ReplanSlotRow`) | i13 재계획 슬롯 행 + VM 타입(TRIP-563). `ReplanDraftScreen`·`_dev/preview.tsx`(`ReplanSlotVM` 타입)가 소비. 옛 자리 `features/planb/ui/ReplanSlotRow.tsx`는 TRIP-810으로 삭제됨(전부 entities 경로로 재조준) |
