...
| `buildDraftDayTabs` · `formatDraftDayHeader` · `buildDraftPins` · `shouldKeepPollingDraft` · `resolveDraftView` · `DraftDayTab` · `DraftPin` · `DraftView` · `DRAFT_POLL_INTERVAL_MS` · `DRAFT_POLL_MAX_COUNT` | `features/itinerary/model/draftView` | **신규(TRIP-297)** — h11 순수 판정 5함수 + 판별 유니온 3타입 + 상수 2개(폴링 간격·상한, 정본 부재·발명값). `DraftPage.tsx`가 유일한 소비자 |
...
| `resolvePlanState` · `formatNightsLabel` · `buildPlanDayTabs` · `PlanState` · `PlanDayTab` | `features/itinerary/model/planState` | **신규(TRIP-299)** — h25 순수 판정 3함수 + 판별 유니온 2타입. `ItineraryPlanPage.tsx`가 유일한 소비자. `formatDraftDayHeader`(위 `draftView` 행)는 카드 위 날짜 헤더 표시에 그대로 재사용됐다(같은 feature 내 재사용, 새 포맷터 미신설) |
| `coPickProgress` | `features/itinerary/model/coPickProgress` | **신규(TRIP-335 슬라이스2)** — h16 허브 진행 카운터 순수함수. `{total,filled,remaining}` 반환, 고정 슬롯은 항상 `filled`로 접힘(PBT로 `filled+remaining=total` 항등 검증). 구조 가드(G2)가 화면의 재계산을 금지 — `CoPickHubPage.tsx`가 유일한 소비자 |
| `formatRadiusUsed` | `features/itinerary/model/radiusUsedLabel` | **신규(TRIP-335 슬라이스2)** — 서버 `radiusMUsed`(미터, 숫자)를 "약 X.Xkm"/"약 Nm" 문자열로 포맷(`legDistance` 선례 재사용). 요청 반경이 아니라 응답값만 받는다(INV-2). `SlotFillPage.tsx`가 유일한 소비자 |
