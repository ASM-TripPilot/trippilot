...
## `src/features/trip/` — 계약 계층 + 위저드 1/2 화면 + 제출·오류 표면 (TRIP-203·204·205·206)

**여행 생성 위저드 1/2이 TRIP-205로 처음 생겼다** — `ui/`가 이번에 신설돼 `model/`뿐이던 슬라이스가 화면째가 됐다. 서버 계약(orval `trips`·`preferences` 태그)을 도메인 이름으로 감싸는 얇은 층(TRIP-203)·드래프트 검증(TRIP-204)에 이어, 이번 칸은 판정과 렌더를 파일로 가른다 — 순수 계산(`tripWizardStep1.ts`)·상태 상자(`tripWizardStore.ts`, Zustand)·무상태 화면(`TripWizardStep1Screen.tsx`) 셋으로 쪼개 `validateTripDraft`(TRIP-204, 페일클로즈)를 부르는 곳을 배선 층(`pages/trip-new-step1/`) 하나로 좁힌다. **TRIP-206이 그 위에 제출·오류 표면·step2 전이를 붙였다** — `touched`(TRIP-205가 만들고 안 쓰던 상태)를 처음 소비하고, 서버 실패를 `overseas`/`banner` 둘로 분류하는 페일세이프(INV-4)를 더했다(아래 `tripDraft.ts`·`tripWizardStore.ts`·`TripWizardStep1Screen.tsx` 행, 배선 로직은 위 `pages/trip-new-step1/` 절 참고).
...
| `src/features/trip/model/tripWizardStore.ts` | **TRIP-205 신설 → TRIP-206으로 `createdTripId` 추가.** **Zustand 스토어**(`useTripWizardStore`, `create(createTripWizardDraft)` 형태 — 구조 가드 6-2 정합, `preferenceStore.ts` 선례). 여행지·기간·인원·동반유형 드래프트 + `touched`(어떤 축을 건드렸는지 집합, **TRIP-206이 첫 소비자** — 오류 문구 표시 게이팅) 보관. 판정은 안 함(그냥 상태 상자) — persist 없음(모듈 싱글턴이라 재진입 보존은 저절로 성립, AC-11). ⚠️ **`removeDestination`은 5-c에서 `Array.filter`(전체 일치 제거) → `findIndex`+슬라이스(첫 일치 한 자리만 제거)로 수정**됐다 — 고치기 전엔 같은 지역을 두 번 담고 하나만 지우면 **둘 다** 사라졌다(문제로그 참고). **`createdTripId`·`setCreatedTripId`는 TRIP-206 신규**(D7) — 라우트가 `tripId`를 안 나르는데 g02(TRIP-84·TRIP-193) 계약(`POST /trips/{tripId}/bases`)은 요구해서 스토어에 선투자로 담았다. ⚠️ **이 값은 지금 심판이 0건이다**(삭제해도 전 테스트 green) — 유일한 소비자 후보 `step2.tsx`는 승인 심판(P-2)이 `@/features` import를 금지해 원리적으로 못 만든다. 개념 [[Zustand 스토어]]·[[touched 상태]] |
...
| `useTripWizardStore` · `TripWizardDraft` · `TripWizardField` | `features/trip/model/tripWizardStore` | 위저드 1/2 드래프트 Zustand 스토어(TRIP-205) — persist 없음, 판정은 안 하는 상태 상자. `touched`는 건드린 축 집합(**TRIP-206이 첫 소비자**, 오류 문구 게이팅). `createdTripId`·`setCreatedTripId`는 TRIP-206 신규(g02 선투자, 심판 0건) |
