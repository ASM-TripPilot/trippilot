...
| `trip-new-step1` | `src/pages/trip-new-step1/ui/TripNewStep1Page.tsx` | **신규(TRIP-205)** — g01 위저드 1/2 배선(121줄). 이 사이클에서 **유일하게 `validateTripDraft`를 부르는 곳** — `destinations.length>0 && startDate/endDate 둘 다 있음 && validateTripDraft(...).length===0`로 `canProceed` 계산. `baseDate` prop 미지정 시 `todayIso()`로 폴백(⚠️ 이 폴백이 실제 프로덕션의 유일한 경로인데 그 반환값을 검증하는 테스트가 없다 — 문제로그 참고). `useTripWizardStore` 셀렉터 6개로 액션을 개별 구독 |
...
| `src/features/trip/model/tripDraft.ts` | **(TRIP-204)** — 위저드 드래프트 검증 순수 함수 3개 + 타입 2개(`TripDraft`·`TripViolationCode`). `validateTripDraft`는 위반 코드 **집합**(중복 없음, 순서 무계약)을 돌려준다 — `NO_DESTINATION`·`END_BEFORE_START`·`NIGHTS_EXCEED_PERIOD`·`PARTY_BELOW_ONE` 4종 고정(계약에 없는 판정은 발명하지 않음, 01b D4). `nightsSum`은 Σ박수, `toCompanionType`은 온보딩 5값→서버 `CompanionType` 4값 매핑(`Map` 기반, `부모님→null`). **TRIP-205가 첫 소비자**(`pages/trip-new-step1/`가 유일하게 부름) — AC-8을 `tripDraftBoundary.test.ts`(전이 의존까지)가 잠근다. **5-c에서 페일오픈→페일클로즈로 반전**(판독 불가 날짜·`NaN` nights가 조용히 "통과"로 무너지던 것을 비교 방향 반전으로 닫음 — `tripDraft.ts:31-39`·`:57-77` 주석 참고) |
...
| `src/features/trip/model/tripWizardStore.ts` | **신규(TRIP-205)** — **Zustand 스토어**(`useTripWizardStore`, `create(createTripWizardDraft)` 형태 — 구조 가드 6-2 정합, `preferenceStore.ts` 선례). 여행지·기간·인원·동반유형 드래프트 + `touched`(어떤 축을 건드렸는지 집합, **아직 소비자 없음** — TRIP-206이 오류 문구 표시에 씀) 보관. 판정은 안 함(그냥 상태 상자) — persist 없음(모듈 싱글턴이라 재진입 보존은 저절로 성립, AC-11). ⚠️ **`removeDestination`은 5-c에서 `Array.filter`(전체 일치 제거) → `findIndex`+슬라이스(첫 일치 한 자리만 제거)로 수정**됐다 — 고치기 전엔 같은 지역을 두 번 담고 하나만 지우면 **둘 다** 사라졌다(문제로그 참고). 개념 [[Zustand 스토어]]·[[touched 상태]] |
...
| `validateTripDraft` · `nightsSum` · `toCompanionType` · `TripDraft` · `TripViolationCode` | `features/trip/model/tripDraft` | 드래프트 위반 코드 집합 판정(순수·PBT numRuns 500) · Σ박수 · 온보딩→서버 동반유형 매핑(TRIP-204). AC-8이 UI·쿼리 훅·라우터 import를 전이까지 금지. **TRIP-205가 첫 소비자**(`pages/trip-new-step1/`가 유일하게 부름) |
...
| `useTripWizardStore` · `TripWizardDraft` · `TripWizardField` | `features/trip/model/tripWizardStore` | 위저드 1/2 드래프트 Zustand 스토어(TRIP-205) — persist 없음, 판정은 안 하는 상태 상자. `touched`는 건드린 축 집합(아직 소비자 없음 — TRIP-206 몫) |
