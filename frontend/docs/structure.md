...
| `presetRange` · `PERIOD_PRESETS` · `COMPANION_OPTIONS` · `PeriodPresetCode` · `CompanionCode` | `features/trip/model/tripWizardStep1` | 프리셋 코드 → 실제 날짜 범위(에포크 일수 계산, `new Date(` 미사용, TRIP-205). 시계 안 읽음(기준일은 인자). **`formatDateRange`·`dayOfWeek`는 TRIP-808로 `entities/trip/lib/formatTripPeriod`로 이관** — 이 파일은 그 둘을 내부(`presetRange`)에서도 쓰므로 `import`+로컬 재수출(순수 재수출 아님, 위 entities/trip 행 참고) |
...
| `buildSlotKey` · `parseSlotKey` · `buildSlotKeys` · `ParsedSlotKey` · `SlotKeySet` | `entities/itinerary-slot/lib/slotKey`(TRIP-809 이관, 원래 `features/itinerary/model/slotKey`) | slotKey 조립·파싱·목록 충돌 검출(BR-U2-04, 판별 유니온 반환, TRIP-295). **바이트 그대로 이관**(파일째 이사). 옛 자리는 전 export 재수출 shim(28개 importer 무수정, TRIP-810 정리 예정) |
| `resolveCategoryPlaceholder` · `CategoryPlaceholder` | `entities/itinerary-slot/lib/categoryPlaceholder`(TRIP-809 이관, 원래 `features/itinerary/model/categoryPlaceholder`) | 카테고리→`{tintClass, iconKey}` 순수 매핑(7종+폴백, TRIP-465). **바이트 그대로 이관**. 옛 자리는 재수출 shim(런타임 소비처 0, orphan — TRIP-810 정리 예정) |
| `SlotPhotoPlaceholder` | `entities/itinerary-slot/ui`(TRIP-809, 원래 `features/itinerary/ui`) | h25 사진 없는 슬롯 카테고리 틴트+아이콘 플레이스홀더(TRIP-465). `TimelineScreen`이 소비. 옛 자리는 재수출 shim(런타임 소비처 0, orphan) |
| `PoiSlotCard` | `entities/itinerary-slot/ui`(TRIP-809, 원래 `features/itinerary/ui`) | 통일 POI 카드(peek/list, TRIP-301). `TimelineScreen`이 소비. 옛 자리는 재수출 shim(런타임 소비처 0, orphan) |
| `ReplanSlotRow` · `ReplanSlotVM` · `SlotBadgeKind` | `entities/itinerary-slot/ui`·`model`(TRIP-809, 원래 `features/planb/ui/ReplanSlotRow`) | i13 재계획 슬롯 행 + VM 타입(TRIP-563). `ReplanDraftScreen`이 소비, `_dev/preview.tsx`는 옛 자리 shim에서 `ReplanSlotVM` 타입만 소비(TRIP-810이 이관 예정) |
