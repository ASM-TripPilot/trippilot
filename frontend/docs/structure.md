...
## `src/pages/` — FSD pages 층 (TRIP-173 신설, 10슬라이스 — TRIP-181로 `stay-search`·TRIP-183으로 `region-picker`·TRIP-198로 `stay-register`·TRIP-205로 `trip-new-step1`·**TRIP-221로 `place-explore` 추가**)
...
## `src/features/explore/` — 실구현 ⑤ (TRIP-183, 이번 사이클 문서 소급 반영 · 담기 토글 TRIP-220 · d04 화면 TRIP-221 · d04 상태·예외 TRIP-222)

d1b·e00 지역 선택 + '내 주변' 진입점 + **d04 장소 탐색 화면(TRIP-221)** + **d04 상태 5얼굴·담기 실패 표면(TRIP-222)**. 배선은 각 화면의 `pages/` 슬라이스가 갖는다(`region-picker`·`place-explore`). **TRIP-220으로 담기 토글 서버 상태 계층이 처음 생겼고, TRIP-221이 그 첫 소비자다** — `savedPlaces.ts`에 `savedPoiIds: string[]` 반환 한 필드만 추가(Seed Q2, 기존 3개는 시그니처·동작 불변). **TRIP-222가 `resolvePlaceListState`(판정)·`placeSaveGuard`(좌표·실패 사유)를 더해 `SavedPlacesOutcome`을 페이지가 처음으로 읽는다**(이전엔 `void`로 버렸다).
...
| `useSavedPlaces` · `SavedPlacesOutcome` · `SavedPlacesFailureReason` | `features/explore/model/savedPlaces` | **신규(TRIP-220) → TRIP-221이 첫 소비자 → TRIP-222가 반환값을 처음 읽음.** 담기 토글 훅. 낙관적 업데이트(리포 최초 패턴) + 판별 유니온 결과(`{kind:'saved'\|'removed'}`\|`{kind:'failed',reason}`, reason은 `unauthenticated`\|`saved-id-unknown`\|`not-found`\|`network` 4갈래). `savedPoiIds: string[]`는 TRIP-221 추가(CTA 개수 출처). **TRIP-222부터 `PlaceExplorePage.attemptToggle`이 이 반환을 소비**해 `SAVE_FAILURE_NOTICE[reason]`을 배너로 표면화한다(이전엔 `void`로 버려 실패가 사용자에게 안 닿았다) |
...
| `resolvePlaceListState` · `PlaceListState` | `features/explore/model/placeListState` | **신규(TRIP-222)** — 판별 유니온(5종, `filter-zero`는 `blame` 포함) + 판정 순수 함수(PBT 대상, numRuns 500). 우선순위 `loading>error>results>filter-zero>empty`. 화면은 이 결과를 받기만 하고 재판정하지 않는다(구조 가드 AC-G3) |
| `hasUsableCoords` · `SAVE_FAILURE_NOTICE` · `COORD_BLOCKED_NOTICE` · `PlaceSaveNotice` | `features/explore/model/placeSaveGuard` | **신규(TRIP-222)** — 좌표 유효성 순수 술어(PBT) + 담기 실패 4갈래·좌표 차단 안내 문구 테이블(`Record` 전수 강제로 갈래 증가 시 tsc가 먼저 깨짐, AC-14) |
| `StateNotice` · `StateNoticeAction` | `shared/ui/StateNotice` | **승격(TRIP-222, `git mv` — 내용 무변경)**, 원래 `features/stay/ui`(TRIP-182). `empty`·`filter-zero`·`error` 3상태 공용 안내 블록, props 6개로 완전 파라미터화. 호출부: `stay`·`explore` |
