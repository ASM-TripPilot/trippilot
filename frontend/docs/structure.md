...
| `place-explore` | `src/pages/place-explore/ui/PlaceExplorePage.tsx` | **신규(TRIP-221)** — d04 배선(66줄). `useGetPlaces({region?,category?})`(region은 라우트 파라미터, 없으면 생략 — Seed Q9, `'부산'` 폴백 미채택) + `useSavedPlaces({isAuthed})`(TRIP-220, `isAuthed=getAccessToken()!==null` 동기 판정 — Seed Q1)로 두 서버 상태를 받아 `visiblePlaces(data, searchText)`로 정렬·검색을 끝낸 뒤 화면에 내린다(화면은 재판정 안 함). 카테고리·검색어는 **페이지가 전부 소유**(화면 `useState` 금지가 기계 강제). `handleToggleSave`가 `isSaved(poiId)`로 담기/해제를 가르지만 **`save`/`remove`의 판별 유니온 반환을 `void`로 버린다**(code-critic W-2 — 응답 대기 중 같은 카드를 다시 누르면 그 두 번째 의도가 조용히 사라짐, TRIP-222 이관). `onBack={() => router.back()}`(5-c, W-3 반영) |
...
## `src/features/explore/` — 실구현 ⑤ (TRIP-183, 이번 사이클 문서 소급 반영 · 담기 토글 TRIP-220 · d04 화면 TRIP-221)

d1b·e00 지역 선택 + '내 주변' 진입점 + **d04 장소 탐색 화면(TRIP-221)**. 배선은 각 화면의 `pages/` 슬라이스가 갖는다(`region-picker`·`place-explore`). **TRIP-220으로 담기 토글 서버 상태 계층이 처음 생겼고, TRIP-221이 그 첫 소비자다** — `savedPlaces.ts`에 `savedPoiIds: string[]` 반환 한 필드만 추가(Seed Q2, 기존 3개는 시그니처·동작 불변).
...
| `src/features/explore/model/placeListView.ts` | **신규(TRIP-221)** — 순수 함수 `visiblePlaces(places, searchText)`(19줄, AC-7·AC-8). `nameKo` 부분일치 필터(빈 문자열이면 전체 반환) 후 `savedCount` 내림차순 정렬(`Array.prototype.sort`는 안정 정렬이라 동점은 입력 순서 유지). 캐시가 소유한 배열을 제자리 정렬하지 않고 항상 새 배열 반환 — 계약(`GET /places`)에 정렬·검색 파라미터가 없어 클라 가공을 이 한 곳에 모은 것 |
| `src/features/explore/ui/ExploreGlyphs.tsx` | 인라인 SVG 4종(백 셰브론·검색·주변 핀·정보) → **TRIP-221로 하트 3종 추가**: `HeartOutlineGlyph`(미담김)·`HeartFilledGlyph`(담김)·`HeartBadgeGlyph`("담음" 배지 안 축소 하트). Figma에서 path 실측(근사 없음) — `StayGlyphs.HeartOutlineGlyph`가 이미 있지만 features 간 직접 import 금지 관례(`stay`도 `explore`의 글리프를 안 쓰는 것과 같은 선택, structure.md L197 명시)로 새로 그렸다 |
...
| `src/features/explore/ui/PlaceExploreScreen.tsx` | **신규(TRIP-221)** — d04 장소 탐색 무상태 프레젠테이션(314줄, prop 9개: 8개 확정 계약 + 5-c `onBack?`). `SafeAreaView(edges=['top'])` → `AppBar` + `FlatList(numColumns=2)`(RN에서 2열을 만드는 유일한 방식 — flex-wrap 한 겹 구현은 렌더 테스트가 거부하도록 설계됨) + 조건부 `CtaBar`(스크롤 밖 형제, 담은 곳 0곳이면 미렌더). 카테고리·검색어 `useState` 0건(상태는 페이지 소유, 구조 가드로 잠금). **정렬 칩은 "요즘 담긴 순" 1개만 렌더**(Seed Q8 — 계약 정렬 재료가 `savedCount` 하나뿐이라 "지금 뜨는 순"을 더 그리면 완전히 같은 순서의 no-op 컨트롤이 된다) · **검색바 필터 아이콘 미렌더**(Seed Q5 — 동작 규정이 정본에 없음, 심판은 버튼 인벤토리 1건뿐이라 장식용 non-Pressable 아이콘은 못 잡는다) · **"가까운 순" 미렌더**(계약에 좌표 파라미터 없음). `imageUrl` null이면 회색 박스(`bg-surface-soft`) — 그라디언트 틴트 미채택(`REGION_TINT`는 6종 손수 배색이라 카테고리 7종에 재사용 불가, 근거 없는 배색 발명 회피) |
...
| `useSavedPlaces` · `SavedPlacesOutcome` · `SavedPlacesFailureReason` | `features/explore/model/savedPlaces` | **신규(TRIP-220) → TRIP-221이 첫 소비자.** 담기 토글 훅. 낙관적 업데이트(리포 최초 패턴) + 판별 유니온 결과(`{kind:'saved'\|'removed'}`\|`{kind:'failed',reason}`, reason은 `unauthenticated`\|`saved-id-unknown`\|`not-found`\|`network` 4갈래). **TRIP-221로 반환에 `savedPoiIds: string[]` 추가**(CTA 개수 출처). 반환값을 페이지가 `void`로 버리는 소비처가 있다(`saved-id-unknown` 갈래 — 진행 중 재누름이 조용히 무시됨, TRIP-222 이관) |
...
| `visiblePlaces` | `features/explore/model/placeListView` | **신규(TRIP-221)** — `nameKo` 부분일치 검색 + `savedCount` 내림차순 정렬 순수 함수. 캐시 배열 제자리 정렬 금지(항상 새 배열 반환) |
