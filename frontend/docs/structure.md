...
## `src/pages/` — FSD pages 층 (TRIP-173 신설, 9슬라이스 — TRIP-181로 `stay-search`·TRIP-183으로 `region-picker`·TRIP-198로 `stay-register`·**TRIP-205로 `trip-new-step1` 추가**)
...
## `src/features/explore/` — 실구현 ⑤ (TRIP-183, 이번 사이클 문서 소급 반영 · 담기 토글 TRIP-220)

d1b·e00 지역 선택 + '내 주변' 진입점. **컨테이너 없음** — 배선은 `src/pages/region-picker/`(위 절)가 갖는다. **TRIP-220으로 담기 토글 서버 상태 계층이 처음 생겼다** — 화면은 아직 없다(TRIP-221~223이 첫 소비자), 이 칸은 훅·역인덱스 2파일뿐.
...
| `src/features/explore/model/savedPlaces.ts` | **신규(TRIP-220)** — 담기 토글 훅 `useSavedPlaces({isAuthed})`. **리포 최초 낙관적 업데이트 패턴**(`onMutate`/`setQueryData`/`cancelQueries` 계열의 첫 사용 — 이전엔 `useCreateTrip.ts` 하나뿐인 "성공 후 무효화"만 있었다). 성공·409(이미 담음, INV-U1-04)는 `GET /saved-places`·`GET /places` 두 쿼리만 무효화(BR-U1-06, savedCount 파생 근거) — 404·네트워크 실패는 스냅숏 롤백만 하고 무효화하지 않는다(재조회가 서버 값으로 덮어써 롤백 단언이 공허해지는 것을 막기 위해서다). 인증 판정은 훅 밖 주입(`isAuthed`, `resolveNearby` 선례와 동형) — `useGetSavedPlaces`는 `enabled: isAuthed` 가드로 미로그인 시 조회 자체를 안 보낸다. ⚠️ **동시에 다른 두 카드를 토글하면, 늦게 실패한 쪽의 롤백(스냅숏 통째 복원)이 그 사이 확정된 다른 카드의 담기를 캐시에서 지운다**(자가 치유 안 됨 — W-2, 후속 티켓 후보. "지금 작업하려면" 절이 아니라 devlog 인수인계 소관) |
...
| `useSavedPlaces` · `SavedPlacesOutcome` · `SavedPlacesFailureReason` | `features/explore/model/savedPlaces` | **신규(TRIP-220)** — 담기 토글 훅. 낙관적 업데이트(리포 최초 패턴) + 판별 유니온 결과(`{kind:'saved'\|'removed'}`\|`{kind:'failed',reason}`, reason은 `unauthenticated`\|`saved-id-unknown`\|`not-found`\|`network` 4갈래). 화면 없음(TRIP-221~223이 첫 소비자) |
