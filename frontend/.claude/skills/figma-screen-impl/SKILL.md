...
- Figma **fileKey + 화면 노드 ID**. 모르면 spec-perception으로 카탈로그/`get_metadata`에서 찾는다. (TripPilot fileKey·밴드는 기억 `[[figma-wireframe-file]]`.)
- Figma MCP 도구는 deferred → ToolSearch로 로드: `get_screenshot`·`get_design_context`·`get_variable_defs`·`download_assets`.
...
- ⚠️ **`get_variable_defs`가 대개 빈 `{}`** — 이 파일 화면 노드엔 Figma 변수가 미바인딩이다. → **design_context의 raw 값을 정본**으로 쓴다(변수 이름을 기대하지 말 것).
- 노드 메타/컨텍스트가 크면(수백 KB~MB) 파일로 저장됨 → **서브에이전트/jq로 증류**해 main 컨텍스트 오염을 막는다.

### 2. 토큰 스냅

raw 값을 `[{type,value,role}]` 배열(JSON)로 모아 스냅퍼를 돌린다:

```
node .claude/skills/figma-screen-impl/scripts/token-snapper.cjs <values.json>
# type: color | space | radius | font,  value: "#ff385c"|"16"|"22"
```

- **OK(정확)** → 그 토큰 클래스 사용(`bg-primary`·`text-ink`·`rounded-button`·`text-hero`). 동률 후보가 나오면(예 12px=button/input/thumb) **role로 선택**.
- **~(스냅)** → 근접 토큰(오차 표시 검토 후).
- **MISS** → 두 갈래로 판단: (a) **그라디언트·브랜드색**(카카오 노랑·네이버 초록) = 토큰 아님 → raw 또는 전용 토큰 추가. (b) **일회성 레이아웃 값**(gap 14·pt 64 등) = 임의값 `gap-[14px]` 유지가 맞다(디자인시스템 토큰이 아님).

스냅퍼가 "토큰 쓸 값"과 "임의/브랜드 값"을 갈라주므로, 사람은 미스만 판단한다.
