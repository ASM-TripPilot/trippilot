...
- **관측(대상 화면마다 다시 확인)**: 화면 노드는 대부분 변수 미바인딩(`get_variable_defs`→`{}`)이라 **raw + 스냅퍼(2단계)가 실작동 경로다.** 공통 컴포넌트는 DS 섹션 `1230:1045`에 있고 새 화면은 그 인스턴스로 그려진다(`figma-build/references/design-kit.md` §0·§10) — `get_design_context`가 인스턴스 이름을 주면 대응 코드 컴포넌트부터 찾는다. `search_design_system`/`get_code_connect_map`은 게시 라이브러리·Code Connect가 없어 빈 결과가 정상이다.
...
**반경은 예외다** — Figma가 먼저 card 12·선택 칩 8로 바뀌었고 코드는 card 16·pill 999로 동기화 대기다(`figma-build/references/design-kit.md` §3). 반경 스냅 결과를 그대로 쓰지 말고 킷 §3과 대조해 드리프트로 브리프·리포트에 적는다.
...
- **testID 보존** — 기존 화면 컴포넌트의 testID를 그대로 유지한다. 프레젠테이션만 바꾸고 **동작 계약은 불변** → dev-cycle 행위 테스트가 수정 없이 green. testID를 바꾸면 test-designer가 확정한 테스트가 red가 되고 implementer는 테스트를 고칠 수 없다(계약 파기).
...
1. **대상 화면 조준** — 프리뷰 하네스(`src/app/_dev/preview.tsx`)가 딥링크 `trippilot://_dev/preview?state=<키>`로 화면을 격리 렌더한다(알 수 없는 키는 splash로 폴백).
2. **앱 스크린샷** — `scripts/dev-preview-capture.sh <키>`(cwd `frontend/`, 부팅된 시뮬레이터·설치된 앱·Metro 필요)가 키마다 재기동→진입→캡처→상단 dev 오버레이 크롭까지 한다. 출력 `_workspace/preview-capture/<키>.png`. 오버레이가 상단 ~156pt를 덮으므로 헤더대는 이 경로로 대조하지 못한다(`verify-gates` 자동 육안 게이트의 한계와 같다).
3. **Figma 기준** — `get_screenshot(node, contentsOnly:true, maxDimension:1600)` → 반환된 URL을 **즉시** `curl -sL -o "<스크래치패드>/fig-<키>.png" "<url>"`. ⚠️ 이 URL은 **단시간 만료**다(에셋 URL의 7일과 다르다 — 훨씬 짧으니 바로 받는다).
...
- [ ] 기존 testID 1:1 보존 (행위 테스트 수정 없이 green)
...
- **[구현](implementer)**: 3~5단계로 화면을 짓는다. **testID 보존이 행위 테스트를 green으로 유지하는 계약**이다.
