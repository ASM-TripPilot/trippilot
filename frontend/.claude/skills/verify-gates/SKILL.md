...
## 자동 육안 게이트 (자율 세션 · 6-b 대체 · TRIP-831)

실기 스모크는 **오케 직영이라 사람이 있어야 한다**(환경 부재를 "코드가 틀림"과 가르는 대화). 자율/야간 세션엔 사람이 없어 6-b 가 늘 SKIP 됐다 — 그 사각을 정적 비전 대조로 메운다. **실기 스모크의 대체가 아니라 자율 세션 전용 보완**이다(사람 있는 세션은 실기 스모크가 정본).
...
**한계 — 실기 스모크보다 사각이 넓다. 넘겨짚지 마라**:
- 정적 캡처다 — 실제 열림·드래그·터치 흡수는 못 본다(실기 스모크와 공통).
