...
- 에스컬레이션: **같은 검사 3회 연속 FAIL 또는 사이클 누적 FAIL 리포트 5개**(원장 기준 — PASS 리포트는 세지 않는다) → 사용자 에스컬레이션 + 문제로그 기록(`에러로그/`는 2026-07-23 폐지·통합). 실패 검사가 번갈아 나와도 누적 상한에 걸린다.
...
- **red 소급 확인 (사이클당 1회 · 2026-07-21 조건부화)**:
  ...
  - **범위를 "신규 테스트가 있는 사이클"로 좁히는 안은 기각했다** — 리팩토링 사이클이야말로 *구현을 옮겼는데 테스트가 옛 경로 목만 보고 green*이 될 수 있는 지점이라, 이 검사가 가장 필요한 곳이다.
