...
- **게이트① 무결성**: `00_gates.md`의 승인 테스트 파일 해시(포맷 후 상태로 박제됨 — `trippilot-dev-cycle/reference/gate-protocol.md` "해시 규약")와 현재 파일의 `shasum`을 대조. 포매터는 멱등이므로 재포맷으로 인한 차이는 존재하지 않아야 한다 — 다르면 게이트 결과와 무관하게 **FAIL + 즉시 에스컬레이션**(심판 변조).
