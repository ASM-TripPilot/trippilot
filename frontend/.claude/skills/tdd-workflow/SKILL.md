...
red(실패 테스트) → 🧑 사용자 검토 ① → green(최소 구현) → 적대적 리뷰 → **refactor** → 🧑 사용자 검토 ② → verify-gates

**refactor가 검토 ② *앞*에 있는 이유**: 승인 해시가 동결되기 전이라 구조를 고쳐도 재승인이 붙지 않는다. 뒤로 밀면 리팩토링마다 재제시가 따라와 아무도 안 하게 된다(실측, 상세는 하네스 변경이력). 대신 refactor는 **검토 ①에서 동결된 승인 테스트를 한 줄도 못 고친다** — 그 선 아래만 다루고, 전체 검증은 뒤의 verify-gates가 한 번에 한다.
...
- [ ] 모든 테스트가 red 상태임을 실행으로 확인(환경 부재 시 "미실행" 명시 + 필요 설정 기록 — red 소급 확인은 [검증]에서 로그 유무와 무관하게 상시 수행된다)
- [ ] 테스트·인프라 파일에 prettier 적용 완료(승인 해시는 포맷 후 상태로 박제된다)
