...
## 3. 반경·간격·그림자
- 반경(Variables `radius/*` = 코드 `borderRadius`): **card 12 · button 12 · input 12 · thumb 12 · pill/chip 8** · sheet-top 24. **크기 규칙: 카드 안 작은 선택 세그(`아무 때나`·`시간 정해두기`, 높이 ~19) = 8 / 상태 배지(고정·필수·같이 고름…) = 12 / 입력·버튼·CTA·카드 = 12 / 일차 칩·헤더 컨트롤 = 999(원래대로, 손대지 않음). 원형(아바타·점·핀·FAB·토글 손잡이)만 999** — 사용자 결정(2026-09-11, "radius 12로 가자": 알약 칩·16 카드·14 CTA가 부풀어 보임). Figma는 변수 `radius/card`=12·`radius/pill`=8 + raw 노드 치환 완료(1차 비원형 999 811·14 182·16 485·18 65 → 12, 2차 높이≤36 773개 → 8, 원형 257 유지 — 상세는 하네스 변경이력). **코드 `tailwind.config.js`·`design-tokens.test.ts`는 아직 card 16 / pill 999** — 동기화 dev-cycle 대기(card→12, pill→8, `rounded-pill`이 원형 17곳에 쓰여 `rounded-full`로 바꿔야 함). 그때까지 코드 쪽 값은 stale.
...
- **지도+시트 골격이 일정 생성(h)·여행중(i)의 기본 화면 문법이다**(2026-09-10~11): 전면 지도 600 + peek 시트(접힘) 또는 지도 띠 110 + 펼침 시트. 앱바 없음 — 뒤로 원 + `N일차` 칩 오버레이는 **지도의 형제 노드**(WebView 터치 흡수). 헤더는 `갈래 · N일차 · 6월 10일(수)` / 우측 `N곳 · x.xkm`. 순서 변경은 길게 누르기(그립 핸들 없음), 썸네일 72, 지도 위 라벨 pill 없음, 지도 출처 `© Kakao`만. 데이터 한 벌 = 1일차 4곳(광안리·황령산·부산시립미술관·웨이브온) + 숙소 해운대 그랜드 호텔.
