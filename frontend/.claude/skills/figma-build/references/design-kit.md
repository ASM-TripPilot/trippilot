...
Variables `1229:1045`(Light `1229:0`) · 컴포넌트 BottomTab `1236:1177` · Icon 세트 `1237:1159` · 카탈로그 섹션 `1230:1045` · 앱 아이콘 `1243:1046`/워드마크 `1243:1051`/반전 `1243:1060`. **2026-09-11 정리: 화면 인스턴스 0건이던 컴포넌트 34종(Button·Chip·Badge·Banner·SearchBar·Card·AppBar·PlaceCard·CTABar·ItineraryItem 등)을 섹션에서 삭제** — 화면의 칩·검색바·앱바는 전부 손그림 프레임이었다. 남은 컴포넌트는 BottomTab·Icon 둘뿐이고, 색·반경 변수와 §7 벡터가 킷의 실체다. **같은 날 2차: 화면에서 실제 반복되던 손그림을 컴포넌트로 승격하고 전 화면 인스턴스로 교체**(섹션 "공통 컴포넌트" 블록) — SearchBar `4462:1368` · BellButton `4462:1375` · FAB `4462:1396`(icon=plus/heart/place/stay) · SaveChip `4462:1402` · HeartButton `4462:1407` · Badge `4464:1544` · NumBadge `4464:1548` · CheckCircle `4464:1553` · DayChip `4464:1561`(state=selected/default) · Toggle `4464:1569`(state=on/off, r999) · NoticeBar `4464:1599`(tone=info/success/warning/error) · CategoryThumb `4464:1644`(category=명소…기본) · MapPin `4465:2467`(state=default/current/upcoming/done, 번호는 `num` 텍스트 오버라이드) · StopIcon `4466:2979`(kind 21종). **새 화면은 이 인스턴스를 쓴다** — 같은 모양을 다시 손으로 그리지 않는다. 교체 매칭은 구조 서명(타입 트리+벡터 크기+색)이라 h 밴드의 `day-chip`(반경은 §3 — 일차 칩 999)·카드류(`stopcard`·`tCard`)는 서명이 달라 손그림으로 남아 있다.
...
## 3. 반경·간격·그림자
- 반경(Variables `radius/*` = 코드 `borderRadius`): **card 12 · button 12 · input 12 · thumb 12 · pill/chip 8** · sheet-top 24. **크기 규칙: 카드 안 작은 선택 세그(`아무 때나`·`시간 정해두기`, 높이 ~19) = 8 / 상태 배지(고정·필수·같이 고름…) = 12 / 입력·버튼·CTA·카드 = 12 / 일차 칩·헤더 컨트롤 = 999(원래대로, 손대지 않음). 원형(아바타·점·핀·FAB·토글 손잡이)만 999** — 사용자 결정(2026-09-11, "radius 12로 가자": 알약 칩·16 카드·14 CTA가 부풀어 보임). Figma는 변수 `radius/card`=12·`radius/pill`=8 + raw 노드 치환 완료(상세는 하네스 변경이력). **코드 `tailwind.config.js`·`design-tokens.test.ts`는 아직 card 16 / pill 999** — 동기화 dev-cycle 대기(card→12, pill→8, `rounded-pill`이 원형 17곳에 쓰여 `rounded-full`로 바꿔야 함). 그때까지 코드 쪽 값은 stale.
...
```
루트 프레임 표준: `resize(390,H); layoutMode='VERTICAL'; primaryAxisSizingMode='AUTO'; counterAxisSizingMode='FIXED'; itemSpacing=0; fills=[solid('#FFFFFF')]` 그리고 정본 페이지 `1228:1045`에 appendChild 후 x/y 배치(밴드 x 간격은 밴드마다 470/450 — `figma-traps`, 이웃 실측).
...
```
> 아래 등록 메모는 **등록 시점 기록**이다. SVG path는 재사용 정본이지만, 함께 적힌 레시피·노드 id·페이지(UX 리뷰 `1630:1083`의 ALT 프레임 등)는 그 뒤 §0(컴포넌트 인스턴스)·§3(반경)·§10(설계 결정)으로 대체됐을 수 있다 — 어긋나면 §0·§3·§10이 이긴다.
...
> (2026-09-08 inew 밴드 inew01 여행중 허브에서 등록: **shield**(Plan-B 상시 감시 상태 행 — Lucide 방패, "무언가 지켜보는 중" 시맨틱) `<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>` stroke 2.1·24 viewBox·round·**muted(#6A6A6A)**. 상태 카드 4행(hairline 위) 좌측 20px + caption muted + chevronR ink, 행 높이 44(탭 타겟). 감시 상태를 "페이지가 아니라 카드 한 줄"로 접는 §10 결정의 시각 요소(변수 없음 상태). **success 인라인 체크**(도착 확인됨 칩)=§6 `check` 재사용 14px·stroke #0E9384(success), 칩 배경 `success-bg`#E4F5F1 + 글자 `success`#0E9384 — 도착 지오펜스 자동 확인 뉘앙스(코랄 `방문 완료`와 구분).)
...
### 7.5 지도
실 앱 지도는 네이버 네이티브 SDK다(TRIP-861). Figma의 지도 이미지는 공급자와 무관한 콘텐츠다.
- 새 화면의 지도는 **자매 화면(같은 밴드 신세대 프레임)의 지도 노드를 복제**해 쓴다. 출처 표기도 자매를 따른다. **지도 출처 표기(© CARTO / © Kakao / © NAVER)는 세대 판정 축이 아니다**(`figma-traps`).
...
- **히어로 photo 카드**: VERTICAL 카드(흰·hairline·SHADOW·반경 §3 card·clip) → photo(FILL·FIXED h150·IMG) + 흰 pill 배지(x/y 절대, 사진은 layoutMode NONE) + body(제목 Noto Bold20 + 메타 muted + progress + 코랄 CTA).
- **숙소/장소 카드**: photo(FILL·반경 §3 thumb) + ♥(우상 x/y) + 배지(좌상) → meta행(제목 + 별점 I Bold13) → loc muted → 가격 I Bold15 + `/박` muted.
- **검색바**: SearchBar 인스턴스(§0)를 쓴다.
- **칩**: Chip 인스턴스(§10 조립 분자)를 쓴다 — 반경·패딩은 §3. 선택=코랄fill 흰텍스트 / 미선택=흰+hairline+ink.
...
- **지도 (§7.5 — 자매 화면 지도 노드 복제 또는 등록 imageHash 임베드)**: ① 맵 프레임(size, layoutMode NONE, **aspect = 이미지 aspect**) → 자매 지도 노드의 fill 복사 또는 §7.5 등록 해시 `IMG(hash)`(새 이미지는 §7 ASSET_SCRATCH fill 복사 경로). ② **동선 = 직선만**(사용자 결정 2026-07-12). 순서대로 스톱을 잇는 **straight 직선 세그먼트**(코랄 #FF385C w2.5~3, round cap; 흰 casing w6 밑) — Vector `vectorPaths`("M x0 y0 L x1 y1 L …" 스톱 좌표만) 또는 createLine. **도로경로·곡선 금지**. **거점 숙소·필수 방문지 지도는 연결선 아예 없음**(핀만, 동선 X). ③ 핀 = `stops` norm×size: **일정 스톱/일반 POI = 코랄 번호·글자 티어드롭**(흰 숫자 Inter Bold·흰 링·그림자, 팁이 좌표). **숙소(거점) 마커 = 코랄 핀 + 흰 `bed` 아이콘(§6)** — "숙" 텍스트 네모 금지, POI와 시각적으로 구분. 핀은 동선 위 z. ④ 축척바(우하) + 출처 표기(좌하 muted 9px, 문구는 자매를 따른다 — §7.5·§10). 지도-우선 화면은 상단 세그[지도|시간표] + 하단 코스 peek 카드.
...
- **밴드**: x 간격·원점은 밴드마다 다르다 — 값은 `rules/figma-traps.md`, 붙일 때는 이웃 프레임을 실측한다. `x≥0` 필터는 첫 프레임을 놓친다. 끝에만 붙이고 중간 삽입 금지(뒤 전부 리플로우). 스트립 밖 노드는 `get_screenshot`이 1×1 → 좌표로 검증. 사진 페인트는 트리 순회가 아니라 id로 수집.
...
- **역할 분리**: 편집 화면 = 조작만 / 일정 화면 = 경고·사실 / AI 경로 = 판단. 편집 화면에 변수 경고를 넣지 않는다. **예외: 슬롯 위반 배지(영업시간 밖 등 BR-U3-13)는 경고가 아니라 그 슬롯의 사실 표시라 편집 화면(h12 편집 모드·h19 직접 짜기·i07)에도 둔다** — 조작 중에 위반을 보여야 고칠 수 있다.
...
- 미결(손대지 말고 알린다): h14·h16(구 h25 계열) 헤더 거리와 커넥터 합 불일치(§traps). i 밴드 현재 위치 마커가 라이브에선 blue `#1659C9`(link)인데 §6·§7.5는 코랄 ring+dot — GPS 관례 blue를 채택할지 결정 대기(채택 시 전용 토큰 필요, `link` 재사용 금지).
- **지도+시트 골격이 일정 생성(h)·여행중(i)의 기본 화면 문법이다**(2026-09-10~11): 전면 지도 600 + peek 시트(접힘) 또는 지도 띠 110 + 펼침 시트. 앱바 없음 — 뒤로 원 + `N일차` 칩 오버레이는 **지도의 형제 노드**(코드의 지도 위 오버레이 배치와 맞춘다 — 네이티브 지도 터치 흡수 여부는 `repo-traps` 지도 절). 헤더는 `갈래 · N일차 · 6월 10일(수)` / 우측 `N곳 · x.xkm`. 순서 변경은 길게 누르기(그립 핸들 없음), 썸네일 72, 지도 위 라벨 pill 없음, 지도 출처 `© Kakao`만. 데이터 한 벌 = 1일차 4곳(광안리·황령산·부산시립미술관·웨이브온) + 숙소 해운대 그랜드 호텔.
- **카드 장소 이름 옆 회색 `›`(muted-soft 14) = 탐색 장소 상세 진입.** 코랄 `다른 후보 ›`(슬롯 교체)와 색으로 구분. 이름 행만 히트 영역(카드 전체 탭은 길게 누르기·시각 칩과 충돌).
- **후보 시트에 `반경 넓히기` 없음.** 슬롯 후보 API가 0건이면 서버가 한 번 넓혀(`radiusMUsed`) 돌려주고, AI 갈래엔 반경 세그가 없다. 후보 밖은 `장소 검색 ›` 링크(15 Bold primary-text) 또는 0건 CTA `장소 검색`. 같이 짜기의 반경 세그(h10)는 사용자가 반경을 고르는 자리라 유지.
- **상태·오류 배너 = 하단 바 v2**(안내·성공·주의·오류 4톤, 흰 배경 358×52 1줄): 코랄/청록 채움 배너·말풍선을 쓰지 않는다. 오류 바(`저장하지 못했어요` + 코랄 `다시 시도`)는 h03 save-error·g01 save-error에 적용, 안내 바는 e05·h05·h07 등. 2줄이면 64로 늘어나므로 부제는 버린다.
- **편집기 화면은 없다** — 편집은 h08 AI 추천안(펼침)의 편집 모드(헤더 `일정 편집 · N일차`, CTA `저장하기`). 완성(h14)·확정(h16)의 `수정` 진입도 같은 모드. 편집 이력·출처 배지(`AI`/`내가 추가`)는 미결.
- **장소 추가 = 지도+시트 한 화면(h13)**: 동선 핀(코랄 숫자)과 후보 핀(흰 hollow, 글자 없음)을 한 지도에, 거리는 기준 슬롯 명시(`③에서 1.1km`·`숙소에서 800m`). 검색과 주변 탐색은 같은 화면의 두 상태. 직접 짜기(h12)에서 카드 사이 `+` 원(24, hairline-strong)을 누르면 `① 다음에 담기`로 열려 그 자리에 삽입.
- **거점 없음 흐름**: h14 거점 없음(숙소 카드·핀 없음 + v2 바 `거점 숙소가 없어요 · 동선 기준 추천 보기 ›`) → h15 동선 기준 숙소 추천(동선 중심 점선 원 + 추천 숙소 핀 + 카드 3 실명 `평균·최대 km` + CTA `…을 거점으로` + `다른 숙소 둘러보기 ›`) → 거점 지정 → h14 거리 계산 중 → h14. 숙소 검색(e02)의 `동선 거리순` 정렬은 e 밴드 후속.
- **i 밴드 FAB = 아이콘 원 52 코랄(edit 24), 뷰포트 바닥 오른쪽 고정**(x 322·y 높이−68), 프레임 직속. 라벨 알약 아님. 허브·변수 감지에만 있고, 하위 시트(위험 상세·재계획 요청·변경 반영)가 열리면 숨긴다. 펼침 시트 하단 여백 66.
- **선택 칩 글자는 `primary`(#FF385C)** — 연배경(`primary-pale`) 위에서도 `primary-text`로 어둡게 하지 않는다(i04 재계획 요청 시트 적용, 상태 배지 규칙과 동일 근거). 다른 밴드의 선택 칩은 순차 반영.
- **상태 배지(필수·고정·같이 고름·AI·AI 추천·추천·작성중·변경됨) = 연배경 칩**: `color/primary-pale` 바탕 + `color/primary` 글자 Noto Bold 11 + 패딩 8×3 + **radius 12**(높이 19 — 사실상 알약). **선택 칩·세그(`아무 때나`)는 radius 8**(일차 칩은 999 — §3) — 상태 배지(읽기 전용 라벨)와 선택 칩(누르는 것)을 반경으로 구분한다(사용자 결정 2026-09-11). 아이콘이 있으면(🔒 고정) 아이콘도 primary, gap 4. 긍정 완료(`완성`)는 `success-bg` + `success`. **글자를 `primary-text`(#C13515)로 어둡게 떨어뜨리지 않는다** — 연배경 위 글자는 액센트 그대로(사용자 결정 2026-09-11, 토스 `받기`·`변동 감지` 대조: 어두운 글자가 "AI 생성물" 인상의 원인). 2026-09-08의 "점+텍스트" 배지는 폐기(당시 반대한 것은 파스텔 pill이 아니라 그 안의 어두운 글자였다). h 행 29개 적용 완료, 다른 밴드·코드(`Badge`류)는 순차 반영.
- **숙소(거점) 슬롯 카드 = 장소 슬롯 카드와 같은 골격**: `[순번 24 | 사진 72 | 텍스트 열: 시간대 칩(체크인 `21:00`) → 이름 › `고정` 배지 인라인 → `저녁 · 숙소 · 변경 불가`]`, 높이 96. 배지를 별도 열로 빼지 않는다(우측 끝으로 밀려 장소 카드와 간격이 달라짐 — 2026-09-11 사용자 지적, h 행 5장 적용).
- **조립 분자 컴포넌트(2026-09-11, DS 프레임 `1230:1045` 하단 "공통 컴포넌트 2")**: `StopCard`(kind=장소 `4473:2288`/숙소 `4473:2289`, set `4473:2290`) · `Conn` `4473:2298` · `SheetHead` `4473:2861` · `AppBar` `4473:2868` · `Chip`(state=default/selected, set `4473:2876`) · `Button`(variant=primary/secondary, set `4473:2884`). 같은 날 앞서 추출된 14종(SearchBar·BellButton·FAB·SaveChip·HeartButton·Badge·NumBadge·CheckCircle·DayChip·Toggle·NoticeBar·CategoryThumb·MapPin·StopIcon)과 함께 **새 화면은 인스턴스로 그린다**. 기존 화면의 손그림 → 인스턴스 교체는 h·i 행부터 순차(미착수).
