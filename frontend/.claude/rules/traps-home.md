...
- **라이브 홈=discovery/planning, postTrip만 여전히 phase 무심판(collecting·upcoming은 TRIP-701로 코드째 삭제됨)** → (TRIP-401로 갱신) `(tabs)/index.tsx`는 더 이상 phase를 안 넘기지 않는다 — 지배(비-ENDED 중 가장 이른) 여행이 있으면 실제로 `planning` 얼굴로 착지하고, 그때만 조건부-자식 `PlanningHome`이 그 여행의 itinerary GET으로 카드 CTA 목적지를 정해 push한다(`resolveHomePhase.dominantTripId`). `HomeScreen.test.tsx`의 버튼-집합 동치(370-AC-4)도 TRIP-401부터 discovery+**planning** 2얼굴을 잰다(T3 AC-6/AC-7) — planning의 hero CTA·브릿지 CTA 죽은 버튼은 닫혔다. **여전히 열려 있는 것**: postTrip 단계는 서버가 그 단계를 줄 계약이 없어(가정 E) 라이브에 결코 안 나오고 프리뷰 전용으로만 존재한다. **TRIP-701 정정(2026-09-17)**: `HomePhase`의 `collecting`·`upcoming` 갈래와 `HomeSections`의 `{kind:'empty'}` 갈래는 타입 자체가 삭제됐다(`homeTypes.ts`) — 이 항목이 예전에 적었던 "collecting에서도 `SpotsSection`이 role="button"+onPress=undefined(무동작 버튼)로 무심판"이라는 사각은 그 phase 자체가 더는 존재하지 않아 함께 소멸했다. collecting/upcoming을 나중에 다시 도입하면 이 사각이 재발할 수 있으니 그때 재확인할 것.
