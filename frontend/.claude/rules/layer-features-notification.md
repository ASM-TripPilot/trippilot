...
| `ui/ToggleRow.tsx` | 라벨 + 푸시·인앱 스위치 2개. **TRIP-780로 공유 승격**: 인라인 Pressable 복제(52×30)를 지우고 `shared/ui/Toggle`(46×28)로 교체 — l06·l05와 함께 3벌째 소비자가 생기며 shared 승격 조건(`features` 간 import 금지)을 넘겨 `shared/ui`가 유일한 공유 자리가 됐다(구 서술 "소비자 1이라 shared 승격 안 함"은 낡음). 푸시 `checked = pushColumnAvailable && value.pushEnabled`(:83) — **이 줄이 DENIED에서 "켜졌다는 거짓말"을 막는 유일한 실렌더 방어선**이다(5-b 경고-1 뮤테이션 실측: `checked={value.pushEnabled}`로 바꿔도 PBT-U6-F2·구조가드 전부 green 유지, `NotificationSettingsScreen.test.tsx`의 5-c 추가 케이스만 문다 — 개념 [[disabled prop과 accessibilityState]] TRIP-607 절 참고). 인앱은 `disabled={false}` 항상(DENIED에서도 조작 가능, BR-U6-16). 셀 `w-[52px]`는 스위치 46과 어긋난 채 남음(TRIP-774 몫으로 미룸). |
