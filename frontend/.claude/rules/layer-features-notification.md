...
| `model/useNotificationInbox.ts` | **신규(TRIP-576, l01).** `useGetMeNotifications`를 얇게 감싼 조회 훅(`{items, isLoading, isError}`). ⚠️ **소비처(`NotificationInboxPage`)가 `isError`를 안 읽는다** — 조회 실패가 `items:[]`로 접혀 "알림 없음" empty 얼굴로 오표시된다(code-critic 03b 경고-1, 후속 티켓 후보). **`markAllRead`(TRIP-773) 추가** — 미읽음 id만 골라 `postMeNotificationsNotificationIdRead`를 `Promise.allSettled`로 순회 → 전부 끝난 뒤 `invalidateQueries`(접두 키) → 전부 성공 여부 boolean 반환. `ponytail:` 주석으로 `/read-all` 미사용을 남김(50건 limit 잘림도 함께 남음 — 새 티켓 후보, 03b 참고-1). |
