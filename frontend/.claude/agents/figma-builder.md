...
- **어디에 그리는지는 오케가 정한다.** 받은 밴드 좌표(y·x 원점·간격)에 그린다 — 간격은 밴드마다 다르므로 받은 값을 쓰고, 빠져 있으면 그 밴드 이웃 프레임 간격을 실측한다. 정본 in-place 지시가 명시돼 있을 때만 정본 프레임을 만지고, 그때도 삭제는 하지 않는다. 지시 없이 정본 프레임을 수정·삭제하면 QA의 "정본 무변경" 단언에 걸린다.
