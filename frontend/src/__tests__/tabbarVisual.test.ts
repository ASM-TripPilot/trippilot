 ...
 *     숫자 고정만 푼다)
 *   - `size={27}` 정확히 2회(활성·비활성 호출부) + `size = 27` 정확히 10회(아이콘 10종 기본값) —
 *     아이콘을 배열/공통 래퍼로 접는 리팩토링이 두 숫자를 흔든다.
 *     완화형: **"`size={27}` ≥ 1 + `size={24}`·`size = 24` 0건"**(A 쪽 부정 단언은 그대로 남는다)
 *   - `h-[84px]`(밴드) — 01b가 Q1(하단 SafeArea 인셋 합산)을 이번 범위 밖으로 이연한 위에 서
 *     있다. Q1이 뒤집혀 밴드 높이가 `84 + insets.bottom` 계산식이 되면 이 클래스 문자열은
 *     정당하게 사라진다
 *   - `px-[28px]`(밴드 좌우 여백) · `h-[64px]`(알약 높이) · `w-[62px]`(탭 폭) ·
 *     `leading-[13px]`(라벨 줄높이) — 03d 스크린샷 대조가 확정한 Figma 값. `h-[64px]`와
 *     `leading-[13px]`는 **M-1 회귀**(알약 높이를 내용물이 정해 2pt 넘치고 위아래 여백이 9/9로
 *     뭉개진 것)를 막는 자리다
 *   - `bg-canvas`(밴드) — **M-2 회귀** 방지. 밴드를 투명으로 두면 react-navigation 기본 테마
 *     배경 `#F2F2F2`가 드러나 Figma 렌더와 한눈에 구별된다. 완화형: 밴드에 `bg-` 토큰이 있기만
 *     하면 통과
 ...
 * 없는 것이다(AND). 이번 사이클의 출생 red(AC-V1·V2·V3)는 전부 제외구에 걸려 세지 않는다 —
 * 출생 red는 마찰이 아니라 가드가 실효함을 증명하는 정반대의 증거다.
...
describe('AC-V1 (+ AC-S1·S2·S4·S5 소스측 회귀 앵커) · Figma 마스터 기하 — 밴드·알약·탭·라벨', () => {
  it('밴드 84/28 · 알약 64/알약반경/표면 · 탭 폭 62 · 라벨 줄높이 13이 소스에 있다', () => {
    // Arrange — 소스 스캔은 읽는 것이 곧 실행이다(렌더 없음).
    const scanned = readScannedSource();

    // Assert — 대괄호 `[` `]`는 정규식 특수문자라 `new RegExp('h-[84px]')`로 매칭하면
    // 이스케이프 없이는 리터럴에 매치되지 않는다(false negative). 그래서 여기는 문자열
    // `includes`만 쓴다. 값을 요소별로 묶어 한 객체로 내는 이유: 실패 diff에 어느 요소의
    // 어느 값이 빠졌는지 한 번에 보인다(줄을 나누면 첫 줄에서 죽어 나머지 상태를 못 본다).
    // 아래 값들의 근거는 03d 스크린샷 대조 실측이다 — M-1(알약 높이)·M-2(밴드 배경)·
    // M-3(알약 배경)이 전부 "개별 변조 후 326/326 green"이던 자리다.
    expect({
      band: {
        height: scanned.includes('h-[84px]'),
        background: scanned.includes('bg-canvas'),
        sidePadding: scanned.includes('px-[28px]'),
      },
      pill: {
        height: scanned.includes('h-[64px]'),
      ...
      tab: { width: scanned.includes('w-[62px]') },
      label: { lineHeight: scanned.includes('leading-[13px]') },
    }).toEqual({
      band: { height: true, background: true, sidePadding: true },
      pill: { height: true, radius: true, background: true, surface: true },
      tab: { width: true },
...
describe('AC-V2 · 아이콘 좌표계 27 + 렌더 크기 27 — 이 사이클의 핵심 가드', () => {
  it('아이콘 10종이 전부 viewBox="0 0 27 27"이고 27px로 그려지며, 옛 24가 한 개도 없다', () => {
    // Arrange
    const scanned = readScannedSource();
    ...
    // 렌더 크기 — `viewBox`는 **좌표계**지 그려지는 크기가 아니다. `size={27}`을 `size={24}`로
    // 되돌리면 아이콘이 24px로 그려져 Figma와 어긋나는데 위 세 숫자는 그대로다(code-critic
    // 실측: 326/326 green). 두 자리를 다 잠근다 — 호출부 2곳(활성·비활성)과 아이콘 10종의
