 ...
 *  (4) `(tabs)/index.tsx`가 실물 `HomeScreen`을 그리고, 나머지 4탭은 껍데기를 유지한다.
 *
...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const ItineraryRoute = require('@/app/(tabs)/itinerary').default;
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
describe('(tabs)/{explore,itinerary,records,my} — 껍데기 유지 (SC-5 · 선제 green)', () => {
  it('홈 외 4탭은 각각 기존 텍스트 껍데기를 그대로 그린다', () => {
    const { unmount: unmountExplore } = render(<ExploreRoute />);
    expect(screen.getByText('탐색')).toBeOnTheScreen();
    unmountExplore();

    const { unmount: unmountItinerary } = render(<ItineraryRoute />);
