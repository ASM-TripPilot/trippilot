 ...
 */
// TRIP-807 · AC-10 · INV-1·INV-3 — entities/stay 층 구조 가드(fs 소스 스캔). 806 entities/place 동형.
// entities 는 shared 만 참조하고 props-only(상태·라우터·query·store 0) 여야 한다. 카드가 이미지 URL 을
// 지어내면(INV-1) · 소요시간을 표시하면(INV-3) 여기서 잡힌다.
//
// ⚠️ stay 는 place 와 결정적으로 다르다 — StayItem·SavedStay 계약에 이미지 필드가 아예 없어(★10) 카드
//    사진은 항상 회색 자리(`bg-surface-strong`)다. 그래서 G4 긍정 짝을 place 의 `imageUrl` 이 아니라
//    `surface-strong`(회색 자리 실재)으로 둔다.
//
...
describe('G0 · 탐지기 자가검사 — 이게 통과해야 아래 소스 스캔이 의미를 갖는다', () => {
  it('주석 속 금칙어는 걷히고, 코드의 URL·자기참조·duration 은 살아남는다', () => {
    const sample = [
      ...
      "const bad = '@/features/stay/ui/Foo';",
...
describe('G1 · entities/stay 3세그먼트 대표 파일 실재(빈 층 공허 통과 차단)', () => {
  it('model·ui·lib 에 대표 파일이 있다', () => {
    expect(fs.existsSync(path.join(STAY_DIR, 'model'))).toBe(true);
    expect(fs.existsSync(path.join(STAY_DIR, 'lib', 'formatPrice.ts'))).toBe(
      true
    );
    const uiFiles = listSourceFiles(path.join(STAY_DIR, 'ui'));
    expect(uiFiles.length).toBeGreaterThanOrEqual(1);
  });
});

describe('G2 · props-only — entities/stay 는 shared 만 참조한다(상태·라우터·query·store 0)', () => {
  it('금칙 import 0 + shared 참조 긍정 짝', () => {
    const sources = readScoped(STAY_DIR);

    // 긍정 짝 — 모집단이 채워졌고 shared(타입 재수출·하트 글리프 등)를 참조한다(빈 층이면 red).
  ...
  it('ui 카드는 로컬 상태를 갖지 않는다(useState·useReducer 0)', () => {
    const uiSources = readScoped(path.join(STAY_DIR, 'ui'));
...
describe('G3 · INV-3 — entities/stay(ui·lib) 에 소요시간 표기 0(거리·가격만)', () => {
  it('duration 식별자 0 + formatPrice 실재 긍정 짝', () => {
    const sources = [
      ...readScoped(path.join(STAY_DIR, 'ui')),
      ...readScoped(path.join(STAY_DIR, 'lib')),
    ];

    // 긍정 짝 — 가격 포맷터가 실재(빈 층 공허 통과 차단).
    expect(sources.some(({ source }) => source.includes('formatPrice'))).toBe(
      true
    );
...
describe('G4 · INV-1 — 카드가 이미지 URL 을 지어내지 않는다(계약에 이미지 필드 0 · 항상 회색 자리)', () => {
  it('ui 소스에 외부 이미지 URL 리터럴 0 + 회색 자리(surface-strong) 긍정 짝', () => {
    const uiSources = readScoped(path.join(STAY_DIR, 'ui'));

    // 긍정 짝 — 카드가 회색 자리(계약에 imageUrl 이 없어 지어내지 않는다, ★10). place 의 `imageUrl`
    // 긍정 짝을 stay 는 못 쓴다 — StayItem·SavedStay 에 이미지 필드가 아예 없다.
    expect(
      uiSources.some(({ source }) => source.includes('surface-strong'))
    ).toBe(true);
