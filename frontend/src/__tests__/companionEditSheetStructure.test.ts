...
/**
 * TRIP-668 동행 편집 시트 소스 층 가드 (AC-5 — 화면 props-only·raw hex 0·INV-3 duration 0).
 *
 * 무엇을 보장하나: `CompanionEditSheet.tsx` 가 ① 소요시간(INV-3)·`duration` 을 안 쓰고(자매 관례 blanket
 * 앵커 — 이 시트엔 시간/거리 축이 없어 자명하지만 회귀 앵커로 둔다) ② raw hex 를 안 쓰며(토큰 경유)
 * ③ 상태·라우터·쿼리·스토어를 직접 물지 않는 **props-only** 컴포넌트임을.
 *
 * 왜 소스 스캔인가: props-only 순수성(useState·router 부재)은 렌더로 관찰할 수 없다 — 소스 층이라야
 * "숨은 훅"을 잡는다(`periodEditSheetStructure.test.ts` 선례 동형). ⚠️ 이 시트가 무상태여야 하는 건
 * 계약이다(01b D4 ↔ D1 텐션을 무상태로 확정, 02a §1) — 드래프트(party·companionType 편집 중간값)는 배선
 * (TripNewStep1Page)이 소유한다. `useState`가 시트로 새면 이 가드가 red 로 잡는다.
 *
...
describe('AC-5 · CompanionEditSheet.tsx 소스 가드', () => {
  ...
  it('g4 · props-only 순수성 — 상태·라우터·쿼리·스토어·타 feature 0건', () => {
    ...
    expect(src).toContain('onSelectCompanion');
