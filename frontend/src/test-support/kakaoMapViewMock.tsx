...
/**
 * `@/shared/map`의 테스트용 목(관찰 마커) — `StayRegisterScreen.test.tsx` 전용.
 *
 ...
 * 금지)을 위반한다. **이 사이클에서 인라인 팩토리로 먼저 시도해 실제로 그 오류를 재현했다**
 * (02a ★5). 그래서 모듈 스코프 파일로 분리하고, 테스트는
...
export function KakaoMapView({ center }: { center: MapCenter }) {
  return <Text testID="map-root">{`${center.lat},${center.lng}`}</Text>;
}
