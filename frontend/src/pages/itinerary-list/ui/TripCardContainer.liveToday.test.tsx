...
// TRIP-751(#682)부터 CONFIRMED 는 날짜 무관 live 로 간다 — 날짜 분기만 재려면 미확정(초안) 일정이어야
// 한다. 초안이면 날짜 분기가 false 일 때 목적지가 draft 다.
describe('AC-4 · 여행 끝난 다음 날 KST 새벽 — 날짜 분기로 live 에 가지 않는다', () => {
  it('KST 2026-08-24 00:30(UTC 08-23 15:30) · 초안 일정 → live 아닌 draft', () => {
    mockUseItinerary.mockReturnValue({
      data: { ...CONFIRMED_ITINERARY, status: 'PLANNED' },
      error: null,
      isPending: false,
      isError: false,
    } as unknown as ItineraryHookResult);
    ...
    expect(mockPush).toHaveBeenCalledTimes(1);
    expect(mockPush).toHaveBeenCalledWith('/trips/t1/itinerary/draft');
  });
});
