 ...
 *
 * 이 파일도 node 버킷이라 `QueryClientProvider`가 없다. TRIP-208이 페이지에
 * `useSavedStays`(→ `useQuery`)를 붙이면 provider를 못 찾고 던져서 `render`하는 it이
 * 전부 죽는다(02a §5-2 실행 확인). 로딩 상태를 돌려주므로 등록 숙소 행은 **글자 없는
 * 자리표시**로 그려지고, 이 파일의 예산 심판은 그대로 유효하다.
 */
 ...
 *
 * 바로 위 `useSavedStays` 목킹과 **완전히 같은 이유**다: TRIP-209가 페이지에
 * `useSavedPlaces`(→ `useQuery`·`useQueryClient`)를 붙이는 순간 provider 없는 이 버킷에서
 * render가 던진다(02a §5-4 실행 확인). 로그인 상태가 아니고 로딩이므로 '꼭 갈 곳' 섹션은
 * **글자 없는 자리표시**로 그려져 이 파일의 텍스트 단언 모집단이 변하지 않는다(02a ★11).
 */
