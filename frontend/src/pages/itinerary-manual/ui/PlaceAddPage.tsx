...
/**
 * h20 배선(TRIP-338·TRIP-502 · AC-5) — `usePlacesInfinite` 로 후보를 받아 화면에 잇고, 추가를 저장으로 잇는다.
 *
 * 검색 규율(AC-5 · TRIP-502, `PlaceExplorePage` 계승):
 *  - **카테고리·검색어를 서버 파라미터로**(`GET /places?category=&q=`). `region` 은 이 화면 라우트에
 *    출처가 없어 안 보낸다(YAGNI, 03 §트레이드오프).
 *  - **검색은 서버가 한다**(`q`) — 클라 필터는 "받아온 페이지 안에서만" 검색이라 결과가 조용히
 *    빠지던 선행 조건(TRIP-502)을 없앤다. 커서 무한 스크롤로 전량 수신도 없앤다.
 *
