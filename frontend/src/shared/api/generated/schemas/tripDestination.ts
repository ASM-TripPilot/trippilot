...
export interface TripDestination {
  ...
  region: string;
  nights: number;
  /** 행정구역 표준코드(`GET /regions` 의 `regionCode`). **주면 이름 조회를 건너뛴다** — 동명이지역(고성군: 강원·경남 / 중구 5곳)은 이름만으로 어느 쪽인지 정할 수 없어 서버가 비워 두는데, 코드가 오면 그 애매함이 생기지 않는다. 생략하면 종전대로 이름으로 찾는다(기존 클라이언트 동작 불변). **목적지로 쓸 수 없는 코드(`selectable=false`)나 없는 코드는 400** — 명시한 값을 조용히 무시하면 사용자가 고른 곳과 다른 지역으로 여행이 만들어진다. */
