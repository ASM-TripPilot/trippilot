 ...
 * (TRIP-807 이관 — features/stay/model → entities/stay/lib. 내용 무변경, 소비처가 같은 함수를 부르면
 *  무수정 green, features shim 재수출.)
 *
 * 무엇을 보장하나: 스냅숏이 없으면(null·undefined·인자 생략) 항상 "가격 미확인"을, 있으면 어떤 정수
 * 금액·어떤 통화 문자열에서도 "{천단위구분}원~" 꼴을 반환한다. 반환값에 duration·분·소요 같은 소요
 * 시간 표현이 없고(INV-3), 같은 입력을 두 번 호출하면 같은 값이 나온다(결정성). 순수 함수(부수효과
 * 없음)라 화면·서버 없이 검증한다. `· 1박` 합성·통화 분기·48h 신선도는 이 함수의 범위 밖이다.
 *
