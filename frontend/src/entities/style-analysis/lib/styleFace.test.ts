...
import { resolveStyleFace } from './styleFace';

/**
 * TRIP-637 · 스타일 분석 정식/임시 판정 — j05(reflection)와 l03 요약카드(settings)가 **같이 쓰는** 한 곳.
 * TRIP-573 `features/reflection/model/styleThreshold.test.ts` 의 resolveStyleFace 블록을 **단언 무변경**으로
 * 옮겨 왔다(바뀐 것은 import 경로 한 줄 — PBT-U5-F4 CI 차단 게이트가 판정을 따라 이사한다).
 *
 * 무엇을 보장하나:
 *  - 🔴 **PBT-U5-F4(CI 차단 게이트)**: `resolveStyleFace` 는 임의의 envelope 에 대해
