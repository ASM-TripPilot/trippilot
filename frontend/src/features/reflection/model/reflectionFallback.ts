 ...
 *  - ① 서버 `card.subtitle`(서버가 이미 `editedCard ?? draftCard` 로 정한 표시본)이 비지 않으면 그대로.
 *  - ② 결측·빈 문자열이면 클라가 방어적으로 `editedCard?.subtitle ?? draftCard?.subtitle` 재조립.
 *  - ③ 그마저 없으면 `statsCard`(0채움)로 조립한 BASIC 문장 — stats 가 결측이어도 늘 문장이 생긴다.
 *
 * ★ 클라 함수는 **1차 결정자가 아니라 빈 화면 방지 최후수단**(맹점①) — openapi `Reflection.card` 주석대로
...
export function resolveDisplayNarrative(res: Reflection | undefined): string {
  ...
  const shown = res?.card?.subtitle;
  if (nonEmpty(shown)) return shown;

  // ② 결측·빈 → 방어적으로 edited ?? draft.
  const edited = res?.editedCard?.subtitle;
  if (nonEmpty(edited)) return edited;
  const draft = res?.draftCard?.subtitle;
