...
// 안 맞아 실측값을 더한다. `MapPinGlyph`의 tone 선례대로 좁은 유니온이고, 범용 색상표는 안 만든다.
const CHEVRON_STROKE: Record<'onPrimary' | 'primaryText' | 'muted', string> = {
...
export function ChevronRightGlyph({
  ...
  tone = 'onPrimary',
  testID,
}: GlyphProps & { tone?: 'onPrimary' | 'primaryText' | 'muted' }) {
  return (
    <Svg
      testID={testID}
      width={size}
