 ...
 *
 * 무엇을 보장하나:
 *  - **★4 · 01b D11** 지정에 성공하면 `bases`를 다시 받아오고 `saved-stays`는 **건드리지
 *    않는다**(coverage 도 무효화 대상이나 TRIP-675 로 조회 훅 `useTripCoverage`·해제 훅
 *    `useUnassignBase`가 고아로 삭제돼 여기서 관찰하는 것은 bases·saved-stays 뿐). 무효화는
 *    낙관적 갱신 대신 고른 길이다 — "지정하면 `blocked`가 어떻게 바뀔지"는 서버 판정이라
 *    클라이언트가 예측하면 INV-2 위반이다.
...
/** 두 조회를 **활성**으로 띄운 관찰용 훅 — 무효화가 실제 재요청으로 이어지는지 보려면 그
...
/** 두 조회가 각각 정확히 1회 다녀온 상태를 만든다 — 증가 단언의 기준선이다. */
...
describe('★4 · 01b D11 — 지정은 bases 를 다시 받아오되 saved-stays 는 안 건드린다', () => {
  it('🔴 지정에 성공하면 bases 를 다시 받아오고 saved-stays 는 안 받아온다', async () => {
...
describe('★10 · 01b D7 — tripId가 없으면 요청이 한 건도 안 나간다', () => {
  ...
  it('짝 — tripId가 있으면 두 요청이 모두 나간다', async () => {
