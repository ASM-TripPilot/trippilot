...
/**
 * TRIP-619 · AC-4(brief)/AC-3(seed) · BR-U5-20·21 · INV-4 — 방문 409 응답을 code 로 갈라
 * 수렴(VISIT_ALREADY_RECORDED)과 해소필요(VISIT_CONFLICT)를 **서로 다른 결과**로 분류한다.
 * `features/itinerary/model/slotSwapError.ts` 선례를 미러링한다.
 *
 ...
 *
 * ★ slotSwap 과 다른 점: slotSwap 은 계약이 code 를 문서화하지 않아 대표값을 **발명**했지만, 방문은
 * openapi 가 실코드를 문서화했다(895·931·2513 — `VISIT_CONFLICT` · `VISIT_ALREADY_RECORDED`). 그래서
 * 아래 케이스는 상수 대신 **문서화된 리터럴을 직접** 넣어 매핑을 계약값에 못박고(상수 뒤바뀜 검출),
 * V2 가 모듈 상수도 그 값과 같은지 별도로 잰다.
 *
