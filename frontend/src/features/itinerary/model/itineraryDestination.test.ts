 ...
 * 정확히 하나의 목적지 토큰(`method`/`generating`/`draft`/`live`)을 낸다. 이 함수 하나가 두
 ...
 *   FAILED→draft(1차분 유효) · COMPLETE+PLANNED→draft(BR-U3-07 초안) · CONFIRMED→live(2026-09-23
 *   제품 규칙 변경: 확정 일정은 날짜와 무관하게 여행 중 화면 — 구 plan(h25) 대체).
 */
...
const CASES: Case[] = [
  ...
  {
    // CONFIRMED = 확정 완성 → 날짜와 무관하게 여행 중 화면(/live).
    label: 'CONFIRMED(확정)',
    input: {
      notFound: false,
      generationState: ItineraryGenerationState.COMPLETE,
...
describe('🔴 itineraryDestinationHref — 목적지 토큰 → 라우트 문자열 표 (AC-1)', () => {
  it.each<[ItineraryDestination, string]>([
    ...
    expect(itineraryDestinationHref('T', destination)).toBe(href);
  });
});
