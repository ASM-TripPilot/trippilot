...
develop ──●────●────●────●────●──────●────────   ← 작업 통합 + CI 검증
          ↖ PR ↖ PR      ↖ PR
feature/TRIP-123-…  fix/TRIP-456-…  feature/TRIP-789-…
```
...
```bash
git checkout -b feature/TRIP-123-login-api develop
git checkout -b fix/TRIP-456-token-expiry develop
git checkout -b chore/TRIP-789-update-deps develop
git checkout -b hotfix/TRIP-999-payment-crash main   # hotfix만 main에서 분기
```

규칙:

- **Jira 이슈 키는 반드시 대문자** — `TRIP-123`은 자동 연결되지만 `proj-123`은 Jira가 인식하지 못합니다.
...
git checkout -b feature/TRIP-123-login-api

# 2. 작업 & 커밋 (커밋 컨벤션은 commit.md)
git push -u origin feature/TRIP-123-login-api

# 3. PR 생성 → 리뷰 → squash merge (pull-request.md)

# 4. 머지 후 (선택): 로컬 브랜치 정리
git checkout develop && git pull
git branch -d feature/TRIP-123-login-api   # 정리하고 싶을 때만
```
...
git checkout -b hotfix/TRIP-999-payment-crash main
```
