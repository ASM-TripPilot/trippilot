...
> **v1.0 확정 (2026-08-25, 사용자 승인)** · 방침: **FE 합의 비차단** — 정본 계약(#334)대로 선행 구현하고 FE 의견은 통합 시점에 반영한다(팀 결정 2026-08-25) — TRIP-538. 출력 계약 정본은 `reflection-template-design.md`(#334) — 본 FD는 계약을 변경하지 않는다.
...
| # | 확정 내용 |
|---|---|
| #1 | **(c′) 1차 미전달** — vision 캡션만 적용. binding_hints는 BE 수요 발생 시 재론 |
| #2 | **인라인 파일(png/jpg)** — 백엔드 사진 저장소 설계(TRIP-478) 의존을 없앤다. 게이트웨이의 원격 URL fetch 지원은 실측 확인됨(2026-08-25) — 백엔드가 URL 저장으로 확정되면 그때 전환 |
| #3 | **현행 유지** — `"rtpl-" + sha256(trace_id)[:16]`. trace_id(=요청 멱등키) 파생이라 재시도 시 같은 id가 나와 백엔드 저장에 유리하고, 결정론이라 테스트가 재현 가능 |
| #4 | **확정** — 캡션 40자·장면 3~8 (잠정 아님). FE 렌더러 의견이 오면 상수만 조정 |
| #5 | **소프트 위반으로 도입** — 지역명·방문지명 부분 포함 또는 브랜드 고정 태그면 허용. 하드로 두면 '#제주여행' 같은 정당한 합성어까지 지운다. 소프트라 랭킹 감점만 하고 태그는 보존 |
...
계약 §4.1의 교체 맵 중 "장면 생략" 항목을 **카드 보존**으로 바꾼다 (팀 결정 2026-08-25):

| ViolationCode | 종전 | 확정 |
|---|---|---|
| `VISIT_REF_OUT` (PHOTO_* 장면) | 장면 생략 | **유효 방문으로 참조 교체** + 캡션 안전 문구 |
| `VISIT_REF_OUT` (표지) | 슬롯 제거 | **유효 방문으로 참조 교체** |
| `EVENT_NOT_FOUND` (입력 이벤트 ≥1) | 장면 생략 | **실재 이벤트로 교체**(enum 정렬 첫) + 캡션 안전 문구 |
| `EVENT_NOT_FOUND` (입력 이벤트 0건) | 장면 생략 | **레이아웃 전환**(사진 있으면 PHOTO_CAPTION, 없으면 STATS) + 캡션 안전 문구 |

캡션을 함께 바꾸는 이유: 원 캡션이 다른 장소를 서술하면 갈아끼운 사진과 어긋나 **거짓이 된다**.
유효 참조 선택은 결정론 — 입력 visits 순서로 미사용 우선, 전부 소진되면 첫 방문 재사용
(중복은 소프트 위반이므로 카드 보존이 우선한다).
