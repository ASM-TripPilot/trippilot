...
```
src/trippilot/c2/
├── facade.py            HybridAssemblyFacade — 시한 인지 체인 + 관측 발행
├── constraints.py       ConstraintChecker — HC1~HC4 순수 함수
├── ortools_assembler.py    OrToolsAssembler — 1차 (CP-SAT/Routing)
├── llm_assembler.py        LlmAssembler — 2차 (LlmPort 소비, 제안→검증→수리)
├── fallback_assembler.py   RuleFallbackAssembler — 최후 (규칙 점수, 항상 해 반환)
...
### 2.1 HybridAssemblyFacade — 본체

```python
class HybridAssemblyFacade:
    def __init__(self, chain: Sequence[AssemblyPort], checker, repair,
                 clock: ClockPort, trace: TracePort, config: AssemblyConfig): ...
...
solve(problem, deadline_ms):
  ...
  for stage in [OrTools, Llm2차, RuleFallback]:
      if 잔여() < stage.required_ms():          # 진입 전 잔여 확인
          trace.emit(FallbackEvent(stage="assembly", from_mode=stage, to_mode=next, reason="deadline"))
          continue                              # 스킵 — 침묵 금지
      result = stage.solve(problem, 잔여())      # 단계에도 잔여 전파 (내부 time limit로 사용)
      if result is not None and validate(result, problem) == []:
          trace.emit(AssemblyRunRecord(...))       # solve_mode = stage
          return result
...
### 2.5 RuleFallbackAssembler (최후) + scorer.py
...
| `SlowAssembler` | solve 중 FakeClock을 소진시키는 스텁 — 단계 스킵·deadline 검증용 |
...
| 지점 | 이벤트 |
|---|---|
| 매 solve 완료 (모든 단계) | `AssemblyRunRecord(solve_mode, elapsed_ms, violations_found, repaired)` |
| 체인 단계 스킵·강등 | `FallbackEvent(stage="assembly", reason="deadline"/"infeasible"/"quality")` |
