...
> U1이 정의한 타입은 **그대로 재사용**한다: `ItineraryProblem` `ItinerarySolution` `DaySolution` `VisitSlot` `FixedBlock` `TimeWindow` `Violation` `SolveMode` `TravelEstimate` `ScoredPoi` `SolverRunRecord` `FallbackEvent`.
> 본 문서는 U2가 **추가로 필요로 하는 타입**만 정의한다. 규칙은 U1과 동일: frozen+slots, tz-aware, 직렬화 왕복(U5-P10), domain은 표준 라이브러리만.

---

## 1. 솔버 설정 (domain/solver_config.py) — AI-D07 반영

| 타입 | 필드 | 불변식 |
|---|---|---|
| `SolverConfig` | `or_tools_limit_ms: int = 3000` · `llm_stage_timeout_ms: int = 2500` · `local_search_min_remaining_ms: int = 3000` · `buffer_min: int = 15` · `speeds_kmph: {WALK:4.0, PUBLIC:20.0, CAR:30.0}` · `safety: {WALK:1.4, PUBLIC:1.5, CAR:1.5}` · `detour_factor: float = 1.3` | 전 필드 양수. **초기값 = AI-D07·G106 확정값** — 실체는 remote config, 이 타입은 주입 컨테이너. 하드코딩 금지 |
...
## 4. LLM 2차 솔버 출력 스키마 (domain/llm_solver.py)
...
- `poi_id ∉ problem.candidates`인 제안 슬롯은 **드롭 + GateDropEvent** (INV-1 — 솔버 경로에도 closed-set 게이트 적용).
...
| `SolverRunRecord` | 매 solve 완료 시 발행 의무 (business-rules §3) |
