...
| `SHARE_CARD_COPY` | 경량(LIGHT) | U6 (j06 공유 카드 캡션·해시태그 — TRIP-429 후속. **에이전트 없음**: 경계→워커 직행(u6-reflect FD §2.1) — 후보 선택·다단 구성·예산 계단·하드 교체를 하나도 쓰지 않는 단발 변환. 게이트는 `reflection_template` 판정 재사용이되 **등급은 전량 드롭**(단발 1건이라 SOFT 감점을 적용할 랭킹 자리가 없다). 폴백: 경계가 결정론 정적 조립) |
