...
> **라우터 vs 워커 (AI-D02)**: `INTENT`(§2.4)는 자연어 진입의 **라우터**로, 의도를 분류해 워커를 고르고 편집 명령·반영 모드를 정한다. 나머지 feature(§2.1~2.3·2.5~2.7)는 **워커**로 각자 판단·생성만 한다. 어떤 feature도 시각·순서를 확정하지 않는다(확정은 어셈블리).
...
**목적**: 일정 슬롯별 추천 이유를 **해시태그 5개**로 생성. 비동기, 사용자 대기 미차단.

> **v0.3.0 (2026-09-24)**: 한국어 2~3문장 → 해시태그 5개 고정(팀 결정, 차후 조정 가능). v0.2.0 은
> 활동·음식 선호 변수 추가(#703)였다. 화면이
> 슬롯 추천 이유를 태그로 보인다. **와이어는 그대로다** — `explanations: {slotKey: string}` 의
> 문자열 자리에 게이트가 태그를 공백으로 이어 붙여 담는다(`"#뷰맛집 #혼자여행 …"`). 백엔드·FE
> 계약 무변경. 차선책 이유(§2.8 ALTERNATIVE_EXPLANATION)는 문장 그대로 — 같은 게이트가
> feature 로 갈라 본다.
...
**OutputSchema** (LLM ↔ 게이트 내부 — 와이어가 아니다):
```json
{
  "explanations": [
    { "poiId": "string", "tags": ["#string", "#string", "#string", "#string", "#string"] }
  ]
}
```
...
**프롬프트 구조 (시스템)**:
```
당신은 여행 일정의 추천 이유를 해시태그로 요약하는 어시스턴트입니다.
확정된 일정의 각 장소에 대해, 사용자 취향에 맞는 추천 이유를 해시태그 5개로 표현하세요.

규칙:
- 반드시 제공된 poiId 목록 안에서만 작성하세요
- 없는 정보를 지어내지 마세요 — 카테고리·상호명·취향 태그에서 알 수 있는 것만 (근거 그라운딩 — nfr §7.5)
- 장소마다 해시태그를 정확히 5개, 서로 다른 것으로
- 해시태그는 #으로 시작, 공백 없이 10자 이내
- 시각·이동시간·소요시간 표현 금지 (INV-3)
- 링크·전화번호·이메일 금지
```
...
```
시스템 프롬프트에 항상 포함:
- 시각·이동시간 수치를 직접 생성하지 마세요 (어셈블리 검증값만 인용)
- editCommand는 "무엇을"만 표현하고, 시각·순서는 어셈블리가 정합니다
- 일정을 단독으로 확정·저장하지 마세요 (반영은 어셈블리 검증 경유)
...
- reason에 시각·이동시간·소요시간을 언급하지 마세요 — 시각은 어셈블리가 정합니다 (INV-3)
...
- 중복 poiId는 첫 등장만 채택
- 생존분은 LLM이 낸 선호 순서 그대로 통과 — 이 순서는 제안일 뿐, 배치·시각 확정은 어셈블리 몫 (INV-2)
