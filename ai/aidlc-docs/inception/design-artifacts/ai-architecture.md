...
- **산출**: 도로 거리(가용 시) 우선 → 직선거리만 있으면 **우회계수 1.3** 보정 → 이동 수단별 **안전계수**(대중교통 ×1.5·도보 ×1.4, 대기·환승·신호 흡수) 적용. 어댑터 순서: 카카오모빌리티(1차) → 네이버(2차) → 직선거리 추정(최종 폴백)
...
C2는 **결정론 코어**다 — 동일 입력이 동일 출력을 낸다. LLM 점수가 없으면 규칙 기반 점수로, 카카오모빌리티가 실패하면 내부 직선거리 폴백으로 **일정 생성을 지속**한다. '해 없음/시간 초과'는 부분 초안 저장 + '이어서 생성'으로 우아하게 저하한다. C2는 **독립 Python AI 서비스**로 배포되며(§4.4, D11 분기), Kotlin 백엔드에서 서비스 간 호출로 접근하므로 C2 앞에도 타임아웃·서킷 브레이커를 둔다(과거 '인프로세스라 외부 다운 없음' 전제는 폐기).
...
|---|---|---|
| (a) 날씨 | 서버 폴링(M11, 1시간) | 강수확률 60%↑ 또는 기상특보 |
