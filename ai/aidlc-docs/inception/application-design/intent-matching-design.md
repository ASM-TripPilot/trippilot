...
[1차. 질문뱅크 매칭 — 임베딩 유사도]
      ...
      |
      +-- top1 유사도 ≥ T_high(설정값 — §5, 현행 0.82) 그리고 top1·top2 의도 일치
...
[2차. LLM 유사 질문 생성 + 재매칭 투표]  (AMBIGUOUS 전용)
      ...
      → 의도별 가중 투표 (유사도를 가중치로)
      |
      +-- 최다 득표 의도의 득표율 ≥ vote_ratio(설정값 — §5, 현행 0.80) → 의도 확정  [VOTED]
      +-- 미달 → 3차로
      |
      v
...
|---|---|---|
| `k` | 5 | 뱅크 top-k |
| `T_high` | 0.82 | 1차 즉시 확정 임계 (초기 0.88 → 2026-09-02 실측 조정) |
| `T_mid` | 0.75 | 2차 진입 하한 |
| `N_paraphrase` | 3 | 유사 질문 생성 수 |
| `vote_ratio` | 0.80 | 가중 투표 확정 임계 (초기 0.60 → 2026-09-08 실측 조정) |

임계값은 하드코딩하지 않고 설정으로 분리 — 오프라인 평가셋(§6)에서 정확도/지연 트레이드오프를 보고 조정한다.

> **T_high 0.88 → 0.82 (2026-09-02, TRIP-653 계측 첫 튜닝)**: 라벨 발화 30건 실측(KURE-v1 임베딩,
> LangSmith `trippilot-intent`)에서 1차 top1 ≥ 0.80 인 in-scope 발화는 전건 정답 의도였고, 0.82 적용 시
> CONFIDENT 전환으로 LLM 호출 -30%·정확도 손실 0. 관측된 최고점 오분류(범위 밖 발화가 뱅크 경계 문장
> "저녁에 맛집 하나 넣어줘"에 0.783)와의 여유 0.037 — **뱅크 검수(§3.3) 전까지의 하한**으로 삼는다.

> **vote_ratio 0.60 → 0.80 (2026-09-08, TRIP-678 — §6 평가셋 88건 × 운영 배정(PARAPHRASE Claude) 실측)**:
> 득표율 분포가 1.0 / 0.73~0.76 의 양봉이다 — 유사질문이 결정론(Claude temperature 0)이면 투표는
> "만장일치" 아니면 "두 의도 동률"로 갈린다. 동률대(0.73~0.76)의 투표 정답률은 3/5 인 반면 3차 LLM 은
> 승격분을 전건 맞혔다. 0.80 이면 동률대가 전부 3차로 가 88/88(0.60 은 86/88), 비용은 LLM 호출 +16%,
> p95 3.4s → 5.5s. 평가셋 라벨이 사람 검수를 거치면 재확인한다. 측정 도구: `scripts/trace_intents.py --eval`.
