...
| **Bedrock LLM 호출** | `langchain_aws.ChatBedrock` | boto3 직접 쓰면 JSON 파싱·재시도·스트리밍·에러 핸들링을 매번 작성. LangChain은 이걸 내장하고 있음 |
| **벡터 스토어 연동 (pgvector)** | `langchain_community.vectorstores.PGVector` | 임베딩 생성 → pgvector 저장 → 유사도 검색의 연결 코드를 추상화. DB 스키마·인덱스 관리도 내장 |
| **임베딩 생성** | `langchain_aws.BedrockEmbeddings` | Titan Embeddings v2 호출 + 배치 처리 + 캐싱을 한 줄로 |
| **ReflectAgent Bedrock 호출** | `langchain_aws.ChatBedrock` | 회고 생성도 Bedrock LLM을 쓰므로 동일한 이유. 호출 인터페이스 통일 |
...
| **Solver (Bedrock 2차)** | 솔버 폴백 체인은 자체 HybridSolverFacade로 제어. LangChain Agent로 감쌀 이유 없음 |
| **M7 후보 풀 생성** | 단순 DB 쿼리 + 필터 파이프라인. LangChain Retriever로 감싸면 오히려 복잡도 증가 |
| **ScheduleAgent** | LLM 호출 자체는 ChatBedrock을 쓰되, 에이전트 로직(판단·폴백·병렬)은 직접 구현. LangChain Agent의 ReAct 패턴이 우리 흐름과 안 맞음 |
...
### ScheduleAgent (직접 구현, ChatBedrock만 사용)
...
class ScheduleAgent:
    def __init__(self):
        ...
        self.llm = ChatBedrock(model_id="anthropic.claude-3-sonnet")
...
langchain-aws = ">=0.2"          # Bedrock LLM + Embeddings
...
LangChain 적용 영역:
  ...
  Bedrock 호출 전체 ← 파싱·재시도·스트리밍 내장으로 보일러플레이트 제거
