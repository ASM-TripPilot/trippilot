...
> C2 어셈블리의 HC1~HC4 검증은 **합격/불합격(hard)** 게이트다.
> 그 위에 결과 품질을 재는 **핵심 평가 축 2개**를 둔다: **최신성**과 **신속도**.
> 어셈블리 검증을 통과한 출력이라도 이 두 축에서 점수화되어 모니터링·릴리스 판단에 쓰인다.
...
```
                    ...
                    C2 assembly.validate : HC1~HC4 (기존, 변경 없음)
...
**F2가 낮을 때의 동작**: F2는 평가 축이지만, CUR-2(지나간 시각)·CUR-4(폐업 포함)는 **hard로 승격**되어 어셈블리 재실행 또는 해당 대안 폐기를 트리거한다 — 나머지는 점수로만 집계.
...
총 지연 = 의도파악 + Plan수립 + Σ(병렬 step의 max)
   step = 정보수집(병렬 max) + LLM 판단 + 어셈블리 + 조립
```
