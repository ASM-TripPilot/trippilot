"""SRC-P1 확장 — OSM 채택 목록 + 야경 이름 규칙 + shop 제외 (TRIP-685).
...
- **채택 목록이지 배제 목록이 아니다.** `_TAG_MAP` 에도 없고 `shop` 키도 없으면
  **무엇이 와도** None 이다 — 실측 비채택 태그(amenity·highway·building·
  `man_made=observatory`), 근접 오타·대소문자·복수형, **라이프사이클 접두**
  (`disused:`·`was:`·`abandoned:`), 임의 유니코드 키/값, 빈 dict 전부.
...
- **`tourism=viewpoint` 는 태그만으로 야경이 아니다.** 제주 실호출 42건에서 진짜
  ...
  양쪽(천문대는 드롭 / 진짜 전망대는 통과)을 실측 표본으로 같이 건다.
- **shop 은 키 존재로 받되 여행지가 아닌 값을 뺀다.** 제외 목록은 None,
  목록 밖은 SHOPPING, `shop` 을 닮은 키(`Shop`·`disused:shop`)는 채택 아님.
...
# 제외 목록은 표에서 유도해 샘플링한다 — 값이 추가되면 자동으로 덮인다.
_EXCLUDED_SHOPS = tuple(sorted(_SHOP_EXCLUDE))
...
def test_표_전제_야경쌍과_비야경쌍이_모두_존재한다() -> None:
    ...
    assert _SHOP_EXCLUDE, "shop 제외 목록이 비었다 — 편의점·미용실이 전부 들어온다"
...
@settings(max_examples=60, deadline=None)
@given(value=st.sampled_from(_EXCLUDED_SHOPS))
def test_pbt_shop_제외목록은_OSM_값_식별자다(value: str) -> None:
    """제외 목록은 Overpass 가 돌려주는 `shop` **값과 정확 비교**된다 —
...
def test_수집_스크립트의_필터가_채택_표와_정확히_대응한다() -> None:
    ...
    **그 축은 조용히 0건**이다 — 매핑은 되는데 애초에 받아오질 않는다.
    ...
    for filt in collect_osm._TAG_FILTERS.values():
        m = re.fullmatch(r'\["([a-z0-9_]+)"(?:="([a-z0-9_]+)")?\]', filt)
        assert m, f"해독 못 하는 Overpass 필터: {filt!r}"
        parsed.add((m.group(1), m.group(2)))

    assert parsed == set(_TAG_MAP) | {(_SHOP_KEY, None)}, (
        "수집 필터와 채택 표가 어긋났다 — 한쪽만 고치면 축이 조용히 빈다"
    )


@settings(max_examples=60, deadline=None)
@given(value=osm_travel_shop_values())
def test_pbt_생성기_전제_여행_shop_값은_제외목록_밖이다(value: str) -> None:
    """생성기 유효성 — 이 값들이 제외 목록에 들어오면 '채택된다' 주장이 거짓이 된다."""
    assert value not in _SHOP_EXCLUDE
...
def test_pbt_채택_목록_밖_태그는_항상_None(tags: dict[str, str]) -> None:
    """**핵심** — 표에 없고 `shop` 도 없으면 드롭이다.

    실측 비채택 태그 · 근접 오타 · 라이프사이클 접두 · `shop` 닮은 키 · 임의
    유니코드 키/값 · 빈 dict 를 한 전략으로 쓸어 넣는다. 여기가 뚫리면 OSM 한국
    전체(수백만 객체)에서 버스정류장·전봇대·주차장이 후보 풀로 들어온다.
    """
    assume(not any(tags.get(k) == v for k, v in _TAG_MAP))
    assume(not tags.get(_SHOP_KEY))
...
def test_pbt_shop_을_닮은_키는_채택이_아니다(key: str, value: str) -> None:
    ...
    `disused:shop=butcher` 는 닫힌 정육점이다 — 쇼핑 축으로 들어오면 안 된다.
    """
...
@settings(max_examples=250, deadline=None)
@given(name=st.one_of(astronomy_names(), osm_non_viewpoint_names()))
...
# ── 4. shop — 키 존재로 받되 여행지가 아닌 값을 뺀다 ───────────────────
@settings(max_examples=200, deadline=None)
@given(value=st.sampled_from(_EXCLUDED_SHOPS), name=korean_place_names())
def test_pbt_제외_목록_shop_값은_None(value: str, name: str) -> None:
    """편의점·미용실·약국·정비소는 여행 일정에 넣을 곳이 아니다.

    `shop=*` 실측 5.9만 건 중 상당수가 이쪽이다 — 안 빼면 쇼핑 축이 생필품으로
    덮인다. 목록을 표에서 유도하므로 항목이 추가돼도 자동으로 덮인다.
    """
...
def test_pbt_제외_목록_밖_shop_값은_SHOPPING(value: str, name: str) -> None:
    """`shop` 은 값이 수백 종이라 **키 존재**로 받는다 — 제외 목록만 예외다.

    우리가 640건뿐이던 축이고 OSM 이 Overture 의 9.1배를 준다. 값을 화이트리스트로
    바꾸면 그 이득이 사라진다.
    """
...
def test_pbt_모르는_shop_값도_SHOPPING(value: str, name: str) -> None:
    """OSM `shop` 값은 계속 늘어난다 — 모르는 값을 드롭하면 축이 야윈다.

    `shop` 축만 '모르는 것은 넣는다' 로 가는 유일한 예외이고, 그 대가가
    `_SHOP_EXCLUDE` 를 사람이 관리해야 한다는 점이다.
    """
    assume(value and value not in _SHOP_EXCLUDE)
    assert map_tags({_SHOP_KEY: value, "name": name}) is PoiCategory.SHOPPING
...
@settings(max_examples=150, deadline=None)
@given(value=osm_travel_shop_values(), name=osm_non_viewpoint_names())
def test_pbt_shop_은_이름_규칙을_보지_않는다(value: str, name: str) -> None:
    """이름 규칙은 야경 전용이다 — 쇼핑은 전망 어휘가 없어도 채택된다."""
    assert map_tags({_SHOP_KEY: value, "name": name}) is PoiCategory.SHOPPING
...
@settings(max_examples=200, deadline=None)
@given(pair=st.sampled_from(_ALL_PAIRS), shop=st.sampled_from(_EXCLUDED_SHOPS),
       name=osm_viewpoint_names())
def test_pbt_제외_shop_이_붙어도_표_항목이_이긴다(
    pair: tuple[str, str], shop: str, name: str
) -> None:
    """반대 방향 — `natural=peak` + `shop=convenience`(정상 매점)는 여전히 NATURE.
    제외 목록이 채택을 **취소하지는 않는다**."""
    assert map_tags(_tags(pair, shop=shop, name=name)) is _TAG_MAP[pair]
...
@given(pair=st.sampled_from(_NIGHT_PAIRS), shop=osm_travel_shop_values(),
       name=st.one_of(astronomy_names(), osm_non_viewpoint_names()))
def test_pbt_야경_이름_탈락은_shop_으로_떨어지지_않는다(
    pair: tuple[str, str], shop: str, name: str
) -> None:
    """천문대 기념품점·동굴 매점 — 야경 이름 규칙이 드롭하면 **거기서 끝난다**.

    `shop` 으로 흘러 SHOPPING 이 되면, 배제해 놓고 다른 축으로 다시 들이는 꼴이다.
    fail-closed 가 이 모듈의 기본값이다.
    """
...
_ANY_TAGS = st.one_of(
    ...
    st.builds(lambda v, n: {_SHOP_KEY: v, "name": n},
              st.one_of(osm_travel_shop_values(), st.sampled_from(_EXCLUDED_SHOPS),
                        st.text(max_size=10)),
              korean_place_names()),
)
...
def test_pbt_매핑은_결정론적이고_표를_바꾸지_않는다(tags: dict[str, str]) -> None:
    ...
    호출이 채택 표·제외 목록을 건드리지 않는 것도 같이 본다 — 표가 호출로 자라면
    '채택 목록' 이라는 성질 자체가 무너진다. 입력 dict 도 불변이어야 한다:
    수집 스크립트가 같은 dict 를 `tags`·`provenance` 로 다시 쓴다.
    """
    tag_before, shop_before = dict(_TAG_MAP), frozenset(_SHOP_EXCLUDE)
    arg_before = dict(tags)
    assert map_tags(tags) is map_tags(tags)
    assert dict(_TAG_MAP) == tag_before
    assert frozenset(_SHOP_EXCLUDE) == shop_before
...
# ── 9. 야경 탈락이 다른 축을 죽이지 않는다 (회귀 고정) ─────────────────
# 첫 구현은 야경 이름 규칙 탈락 시 `return None` 이라, `tourism=viewpoint` 를
# 함께 단 봉우리·해변이 NATURE 로도 못 가고 **통째로 사라졌다**. 전망 좋은
# 정상은 OSM 에서 흔히 공동 태깅되고, NATURE 는 Overture 대비 7.6배라는 OSM
# 채택 근거 그 자체다. 조용한 손실이라 더 나빴다.
#
# 다만 `shop` 으로는 흘리지 않는다 — 값이 수백 종인 catch-all 키라, 배제한
# 레코드를 뒷문으로 들이는 꼴이 된다(§5 의 fail-closed 속성이 그쪽을 지킨다).


@pytest.mark.parametrize(
    "tags,expected",
    [
        ({"tourism": "viewpoint", "natural": "peak", "name": "조경철천문대"},
         PoiCategory.NATURE),
        ({"tourism": "viewpoint", "natural": "beach", "name": "협재해수욕장"},
         PoiCategory.NATURE),
        ({"tourism": "viewpoint", "natural": "peak", "name": "구린굴"},
         PoiCategory.NATURE),
        ({"tourism": "viewpoint", "natural": "peak", "name": "사라오름전망대"},
         PoiCategory.NIGHT_VIEW),
        ({"tourism": "viewpoint", "name": "구린굴"}, None),
    ],
)
def test_야경_탈락은_강한_태그로_흘러간다(tags, expected) -> None:
    assert map_tags(tags) is expected


@pytest.mark.parametrize("shop_value", ["vacant", "no"])
def test_가게가_아닌_shop_값은_후보가_아니다(shop_value: str) -> None:
    """`shop=vacant` 는 비어 있는 점포 자리, `shop=no` 는 "가게 아님"이다.

    통과시키면 TRIP-683 실재 검증이 걸러내려는 대상을 수집이 생산한다 —
    유령 POI 를 우리 손으로 만드는 셈이다. 취향이 아니라 오류다.
    """
    assert map_tags({"shop": shop_value, "name": "무슨가게"}) is None
