...
`intent_question_bank.yaml`은 **v0.7 — seed 125 + 증강 325 + 거부 앵커 35 = 485문장**이다.
...
- **2026-09-19 거부 앵커(v0.7, TRIP-868)**: 7계열 35문장 추가 → 485문장, 위반 0·경계 후보 83(450 때와 동일 — 앵커가 새 경계를 만들지 않았다).
  LLM 호출 70 → 62회 · LLM 실패 9 → 0~1회 · 정확도 84~85/87. 음식추천 계열은 분리 불가로 제외(TRIP-852).
...
**거부 앵커 (TRIP-868)** — 뱅크의 `OUT_OF_SCOPE` 그룹 35문장
- 위임 대상이 아니다. **1차에서 "우리 일이 아니다" 를 확정해 LLM 0회로 거절**한다. 문턱은 `t_high` 그대로 — 규칙을 하나 더 두지 않는다.
- 없으면 딴소리도 13종 중 **가장 덜 먼 곳**에 붙는다(실측: "주식 시장 어때?" → GET_WEATHER, "비밀번호 바꿔줘" → EDIT_SCHEDULE).
- 실측 효과: **LLM 호출 70 → 62회** · LLM 실패 9 → 0~1회(범위 밖 발화가 3차까지 가서 "분류 불가"를 받던 것을 1차에서 끊는다) · 정확도 84~85/87(변화 없음) · **과잉 차단 0**.
- ⚠️ **계열을 새로 넣기 전에 뱅크 최대 유사도를 먼저 잰다. 0.75 이상이면 넣지 않는다.**
  실측 — 금융·환전 0.672 · 계정·설정 0.678 · 기기제어 0.654 · 잡담 0.622 · 일반상식 0.695 · 사진편집 0.707 · 결제·예약 0.730 · **음식추천 0.847(제외)**.
  음식추천은 "저녁 메뉴 추천해줘"와 사람이 쓴 EDIT_SCHEDULE seed "저녁 일정에 식당 한 곳 추가해줘"가 0.813 이라 문턱을 어디에 둬도 정당한 편집 요청을 함께 막는다(TRIP-852).
- ⚠️ **범위 밖은 열린 집합이다.** 전수 커버가 목표가 아니고 흔한 갈래만 덮는다. 커버리지 수치는 계열을 고를 때 평가셋 범위 밖 예시를 이미 본 상태라 **오염된 측정**이다 — 믿을 수 있는 것은 과잉 차단 0 쪽이다.
- 앵커는 `origin: seed` 다. 증강 대상이 아니다 — 열린 집합을 불려 봐야 의미가 없다.

**§6 CI 게이트 — 기록 재생 (TRIP-841)**
- 게이트: `tests/test_intent_eval_gate.py` — 평가셋 정확도가 기준선 아래로 내려가면 **머지 불가**. 외부 호출 0(D37).
- 픽스처: `tests/fixtures/intent_gate/` (임베딩 746 · LLM 70 · meta). 벡터는 fp16 base64 라 jsonl 한 줄 = 문장 하나 — diff 로 읽힌다.
- **뱅크·평가셋·프롬프트·모델 중 무엇이든 바꾸면 기록을 다시 떠야 한다**:
  ```bash
  cd ai
  ...
  ```
  다시 뜬 기록이 곧 리뷰 대상이다 — 정확도가 내려갔는데 기준선을 같이 낮췄다면 그 PR 에서 보인다.
- **게이트가 재는 것은 품질이 아니라 변화다.** 재생은 결정론이라 실행마다 안 흔들린다. 실제 품질은 여전히
  `--eval` 을 최소 2회 돌려서 본다(아래 「측정값을 읽을 때」).
- ⚠️ **미스를 정확도보다 먼저 본다.** 게이트웨이가 모든 예외를 폴백으로 바꾸므로(BR-U4-02) 기록 미스는
  "정확도가 떨어졌다"로만 보인다. 실측으로 한 번 헷갈렸다 — 배정 모델을 잘못 되살렸더니 86/87 이 64/87 이 됐고
  표시는 "정확도 회귀" 한 줄뿐이었다. **모델 이름은 기록 키의 일부다.**
...
**`detailIntro2` 만 보고 "요금이 없다"고 판단하면 틀린다.** `detailInfo2`(반복정보)에 더 있다 —
`infoname`/`infotext` 쌍이라 필드명이 고정이 아니어서 스키마를 훑어도 안 보인다.
실측 도구는 `scripts/probe_fee_sources.py`(엔드포인트 × 전 필드 스캔, 값의 모양으로 찾는다).

| 타입 | 필드 | 엔드포인트 | 채움 | 성격 |
|---|---|---|---:|---|
| 12 관광지 | `입 장 료` | **detailInfo2** | 66.7% | ✅ 방문 1회 입장료 (무료 54% · 유료 12.5%) |
| 14 문화시설 | `usefee` | detailIntro2 | ~51% | ✅ 방문 1회 입장료 |
| 28 레포츠 | `이용요금` | **detailInfo2** | 93.3% | ⚠️ **1박·사이트 요금** |
| 38 쇼핑 | `saleitemcost` | detailIntro2 | 1% | ❌ |
| 39 음식점 | — | 둘 다 | **0%** | ❌ 원천에 없다 |

⚠️ **28 을 `avg_cost` 로 쓰지 마라.** 우리 풀 1,874건이 전부 ACTIVITY 인데 절반이 캠핑장·글램핑이고
`이용요금`이 `펜션 130,000원`·`오토캠핑 30,000원` 같은 **1박 요금**이다. 방문 비용으로 넣으면
예산 계산이 틀린다 — 숙박은 OTA 위임 영역이다. 금액이 있다고 쓸 수 있는 값이 아니다.

⚠️ **음식점은 외부에도 없다.** 카카오·네이버 지역검색, Overture 스키마에 가격 필드가 없고,
OSM 도 실측 기각(관광 축 7,814건에서 `charge` **0.06%** · `fee` 1.05%는 yes/no 불리언).

방문형 입장료 분포(12+14 합본, n=101): 최소 500 · q25 **2,000** · 중앙 **4,000** · q75 **9,450** ·
최대 35,000. 현행 예산 등급 경계 `15,000`·`40,000` 은 **분포 밖**이다(`>=40,000` 0건).

파싱 함정 — 실물에서 걸린 것만:
- `어른 (25~64세) 1,000원` — 괄호 안은 나이다. **`원` 이 붙은 수만** 읽어야 한다
- `※ 무료 <br>- 유아 / 경로` — 유료 기록 안의 무료 문구. "무료"만 보고 0원으로 읽으면 틀린다
- `[개인]` / `[단체]` — 단체가 더 싸다. 대표값은 개인가여야 한다
- 금액이 뽑혀도 입장료가 아닐 수 있다: 실측에서 `대관료 220,000원`(전시는 무료) · `수강료 90,000원` ·
  `1박 70,000원` 이 섞여 나왔다
- ⚠️ **`일반 9,000원` 의 "반"을 강좌명으로 읽지 마라.** 수강료를 거르려고 `반\s*\d`(`서예반 90,000원`)를
  넣으면 **`일반`** 이 통째로 걸린다 — 실측 21건이 이렇게 잘못 제외됐다. `(?<!일)반` 이어야 한다
- ⚠️ **라벨과 금액 사이에 괄호가 끼면 성인 요금을 못 잡는다** — `성인 (20~65세) 12,000원` ·
  `대인 (만 19세 이상) 8,000원` · `일반(1년) 20,000원`. **금액 없는 괄호를 먼저 걷어내고** 라벨을 찾는다
- `5,000원 (녹차 1잔 제공)` 처럼 **나이 구분이 아예 없는 단일 요금제**가 있다. 성인 라벨을 요구하면
  이것들이 빠져 무료율이 부풀려진다 — 라벨이 없으면 개인가 최댓값으로 폴백

> 위 둘은 **제외된 표본을 눈으로 확인해서야** 드러났다. 한국어 라벨은 부분 문자열 충돌이 잦다 —
> 제외 규칙을 만들었으면 **무엇이 제외됐는지 반드시 출력해 보고** 나서 믿는다. 두 버그 모두
> 진짜 입장료를 무료 쪽으로 밀어 **무료율을 부풀리고 유료 표본을 깎았다**(타입 14: 86 → 117건).
