...
`intent_question_bank.yaml`은 **사람 검수를 마친 seed 뱅크 v0.4**다(2026-09-13) — `reviewed: true`, 실 DB 편입 가능.
평가셋 `intent_eval_set.yaml` 도 같은 날 검수를 마쳤다(v1.0, 87문장).
검수 절차: 의도별 문장을 사람이 확인 → 의도 간 경계 문장 제거 → 뱅크 편입 시 타 의도 엔트리와 유사도 ≥ 0.90 중복 검사 통과 필수 (intent-matching-design §3.3).
**앞으로 추가되는 문장(§3.2 ② augmented · ③ mined)은 `reviewed: false` 로 들어와 같은 절차를 거친다** — 로더가 미검수 항목의 적재를 거부한다.
