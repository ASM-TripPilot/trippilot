...
환경변수 (빈 문자열 = 미설정 취급 — GH Actions는 비운 input을 ''로 주입한다):
    ...
    TOURAPI_MAX_CALLS     기본 "500" — 개발계정 일 1,000의 절반(보수적).
                          상한 도달 시 그 시점까지 산출하고 정상 종료 (부분 성공 = 성공)
...
def main() -> int:
    ...
    max_calls = int(_optional("TOURAPI_MAX_CALLS") or "500")
    output = _optional("COLLECT_OUTPUT") or "collected_pois.json"

    adapter = TourApiAdapter(UrllibHttpClient(), key)
    result = collect(
        adapter,
        area_code=area_code,
    ...
    print(f"[collect] 산출: {output} — 게이트 통과 {stats.passed}건 "
          f"(호출 {stats.http_calls}/{max_calls}, 목록 {stats.listed}건, "
