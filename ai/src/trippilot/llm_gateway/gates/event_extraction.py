...
**address 에는 ⑤ 를 걸지 않는다 — 재도입 금지** (2026-09-19 실측).
프롬프트 인젝션 감사가 "가짜 행사 주소로 선호 점수 +0.1478" 경로를 지적해 주소도
`_name_in_snippets` 에 걸어 봤다. 둘 다 틀렸다:
  · 막지 못한다 — 공격 전제가 "공격자가 스니펫을 쓴다"이므로 공격자 주소는 **그 스니펫
    안에 있다**. 대조를 통과한다.
  · 비싸다 — 스니펫은 장소를 통칭("광안리해수욕장 일원")으로 쓰고 모델은 행정 주소
    ("부산 수영구 광안리해수욕장")로 편다. 그 확장이 정상인데 구·시 토큰이 원문에 없어
    걸린다. 현실적 쌍 7건 중 **주소 1건만 생존**했다.
주소 신뢰는 여기가 아니라 지오코딩·근접 부착 쪽에서 볼 문제다.
"""
