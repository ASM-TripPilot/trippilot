...
class PlaceExtractionGate:
    ...
    **빈 결과 = 성공** (TRIP-260 #5): "이 문서에 등록할 만한 장소가 없다"는 정상
    산출이다. 대체 추출 경로도 없어 폴백이 바꿀 것이 없고, 실패로 뒤집으면
    `value=None`이 되어 "0건"이라는 사실마저 지워진다. 환각 방어의 증빙은
    GateDropEvent가 따로 싣는다 — 전량 드롭이어도 침묵이 아니다.
    """
