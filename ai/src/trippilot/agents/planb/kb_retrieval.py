...
# 당시 4 를 고른 이유는 "전건이 필요한 최대 버킷(fatigue 4건)을 정확히 덮는다" 였다.
#
# **그 근거는 죽었다** — A-5(2026-09-12)에 38건이 되며 weather 만 14건이다. 재측정:
#     top_k |  2     3     4     5     6     8
#     컷 전  | 1.000 0.905 0.786 0.714 0.667 0.571
#     컷 후  |  -    0.952 0.881 0.810 0.776   -     (min_score_ratio 0.85)
#
# 그래도 4 를 유지한다. 새 근거는 **오답의 정체**다 — 컷 후 남는 무관 3건은 top_k 로
# 못 고치는 것이었다: ⑴ `canceled` 가 `sit-fullybooked-001` 을 끌어온다(임베딩이 "예약
# 취소"와 "예약 마감"을 못 가른다. A-5 에서 우리가 **일부러 나눈** 구분이고, 그 조언은
# 취소에도 말이 된다) ⑵ `none`("사유 없음")은 매칭할 의미가 없어 0.510/0.507/0.493 —
# 사실상 무작위다. top_k 3 의 0.952 는 덜 봐서 덜 틀리는 것이고, 그 대가로 weather 의
# 정밀도 1.00 짜리 4번째 문서를 자른다. 0.952 는 튜닝 여지가 아니라 의미 바닥이다.
#
...
# ⚠️ 실 문서에서 64 는 **로컬 임베딩 컨테이너를 OOM 으로 죽인다** (2026-09-19 KB-5
# 적재: 5GiB 상한, 10청크 640건까지 가다 exit 137). 시드 문서는 짧아서 안 걸렸다 —
# 위키 도입부(최대 433자)가 섞이면 활성 메모리가 배치 길이에 곱으로 붙는다.
# 배치를 내려도 **못 막는다** — 죽는 자리가 배치 크기가 아니라 누적 문서 수라서다
# (64 에서 640건, 16 에서도 642건). 누수는 **문서당 약 2.7MB**이고 요청이 끝나도 안
# 풀린다. 모델 상주 2.96GiB + 상한 5GiB → 여유 1.9GiB ÷ 2.7MB ≈ 700건이 한 생애다.
# 그 부근에서는 죽기 전에 먼저 **느려진다**(단건 168ms → 14.6초) — 컨테이너는
# `running`·`oom=false` 인데 한 시간 동안 0건 진행한 적이 있다.
# 수천 건은 패스로 나누고 패스 사이에 컨테이너를 재기동한다. 죽어도 청크별 upsert 라
# 진행분은 남고, `load_kb._skip_already_indexed` 가 그만큼을 건너뛴다 — **이 함수의
# 중복 검사는 DB 존재 확인이 아니라 목록 안의 doc_id 중복 검사다**(둘을 헷갈려
# "재실행이 이어받는다"고 적었다가 640건을 다시 임베딩했다).
