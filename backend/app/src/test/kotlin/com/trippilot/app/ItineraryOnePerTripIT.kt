 ...
 * ## 왜 뒤늦게 쓰나
 *
 * 제약은 2.10 부터 있었는데 **그것을 확인하는 테스트가 없었다.** 그래서 이 리포에서 "행이 둘이라
 * 조회가 아무거나 집는 것 아니냐"는 가설이 한 번 세워졌고(TRIP-536 조사), 실제로는 DB 가 이미
 * 막고 있다는 사실을 아무도 코드로 보여줄 수 없었다. 제약이 조용히 사라지는 날을 대비해 못 박는다.
 *
