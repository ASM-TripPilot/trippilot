...
-- now() 는 STABLE 이라 ADD COLUMN 이 테이블을 재작성하지 않는다(PG11+ fast path — 기본값을 한 번 평가해
-- 카탈로그에 둔다). 이 표는 발행된 행을 지우는 배치가 아직 없어 계속 자라므로 그 성질이 중요하다.
...
-- ⚠ V2.32·V2.37 주석이 이 인덱스 이름을 "같은 꼴"의 비유로 **인용한다**. Flyway 는 forward-only 라
-- 그 파일들을 고칠 수 없으니 여기 남긴다 — 그 주석의 `ix_outbox_unpublished` 는 이제 아래 `ix_outbox_due` 다.
