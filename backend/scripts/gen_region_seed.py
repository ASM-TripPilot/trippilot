...
def main(path: str) -> int:
    ...
    # 통합으로 **시도가 사라진 도시**의 옛 관할 자치구.
    #
    # 광주광역시가 폐지되며 그 자리는 12 아래 자치구 5곳이 됐다. 그런데 그 이름들(동구·서구…)에는
    # '광주'가 없어 검색에 안 걸리고, 상위 시도(12)는 옛 전남 전체라 목적지가 아니다(selectable=false).
    # 그 결과 '광주'로 검색하면 **고를 수 있는 것이 경기도 광주시(41610) 하나**가 되어,
    # 광주 여행을 가려던 사용자가 엉뚱한 도시를 고르게 된다 — 조용히 틀리는 경로다.
    #
    # 자치구를 별칭에 매달아 **맞는 답이 목록에 존재하게** 한다. 화면은 sido_name 으로 묶어 보여주면
    # `전남광주통합특별시 > 동구…` 와 `경기도 > 광주시` 가 구분된다.
    MERGED_CITY_DISTRICTS = {
        "광주": ["12210", "12240", "12270", "12300", "12330"],
    }
    ...
    for code, names in RETIRED.items():
        for n in names:
            add(n, code)
    for alias, codes in MERGED_CITY_DISTRICTS.items():
        for c in codes:
            add(alias, c)
