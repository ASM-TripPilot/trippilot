...
| `request_meta` | ✔ | `RequestMeta` 그대로 (`deadlineMs = 25_000` — 2026-09-08 3s→15s→25s: sol 중앙값 6.8s 가 3s×0.7 에 못 들어갔고, 2단 폴백(재시도 모델 몫 35%)이 붙으며 다시 올렸다) | `deadline_ms` 는 `exclusiveMinimum: 0` — 15000 통과 |
