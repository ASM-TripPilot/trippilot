...
class GeocodeService(
     ...
     *
     * **국내강제(INV-U1-12)를 여기서 판정하지 않는다.** 정방향(`/stays/geocode`)도 하지 않으므로 일관되고,
     * 그 불변식이 실제로 걸리는 자리는 **여행 목적지**(TripService)다 — 숙소 좌표가 아니다. 다만 핀은 지도
     * 어디든 찍을 수 있어 정방향보다 노출이 크다는 점은 남는다: 해외 좌표로 숙소를 등록하는 것 자체는 막히지
     * 않고, 그 숙소를 국내 여행에 거점으로 붙이는 단계에서 걸린다. 이 순서를 바꿔야 한다면 별도 판단이다.
     *
