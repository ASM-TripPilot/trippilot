...
**Implementation in progress.** Backend is a working Gradle multi-module app — 15 feature modules, Flyway migrations V1.0–V2.40 + 6 repeatable `R__` seeds, 197 test sources, docker-compose/GHCR CI. Frontend is scaffolded and under active feature work — architecture canon is `frontend/README.md` (TRIP-160), FSD 층 구조에 TS/TSX 소스와 Jest 테스트가 실재한다. `ai/` 도 **실서비스 코드가 있다** — FastAPI 경계 8종이 열려 있고(`/ai/v1/itinerary/{generate,validate,repair,alternatives,explanations,edit}` + `/ai/v1/reflection/{generate,nudge}`), 그중 백엔드가 실제로 부르는 것은 4종이다(generate·validate·repair·explanations — `HttpScheduleAgentAdapter`). Most design content is Korean documents — do not invent build/test/lint results.
