...
- **AI/Assembly (`ai/`):** Python service (FastAPI + LLM gateway + assembly). 열린 경계와 백엔드가 부르는 경로는 위 §What this is 의 두 정본을 본다. 슬롯 후보(`proposeSlotCandidates`)는 `/ai/v1/itinerary/alternatives` 를 **실호출한다** — 로컬 후보풀 폴백(`degraded=true`)은 AI 미도달일 때만이다. (2026-09-16 정정: 종전 "슬롯 후보는 AI 경로가 없다"는 TRIP-463 구현 전 서술이었다.) 백엔드는 기본 `AI_SCHEDULE_MODE=fake` 로 돌고, 실 연동은 `AI_SCHEDULE_MODE=http` + `docker compose --profile full up -d ai`. 왕복 검증은 `LIVE_AI=1 ./gradlew :app:test --tests "*LiveAiRoundTripIT*"`.
