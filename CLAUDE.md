...
- **AI/Assembly (`ai/`):** Python service (FastAPI + LLM gateway + assembly). 경계 8종이 열려 있고(위 §What this is), 백엔드는 그중 4종을 부른다. **슬롯 후보는 AI 경로가 없다** — 백엔드가 로컬 후보풀로 폴백하며 `degraded=true` 로 표시한다(`HttpScheduleAgentAdapter.proposeSlotCandidates`). 백엔드는 기본 `AI_SCHEDULE_MODE=fake` 로 돌고, 실 연동은 `AI_SCHEDULE_MODE=http` + `docker compose --profile full up -d ai`. 왕복 검증은 `LIVE_AI=1 ./gradlew :app:test --tests "*LiveAiRoundTripIT*"`.
