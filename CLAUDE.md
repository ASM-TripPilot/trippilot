...
**Implementation in progress.** Backend is a working Gradle multi-module app — feature modules + Flyway SQL-first migrations with repeatable `R__` seeds, docker-compose/GHCR CI. **세는 값(모듈 수·마이그레이션 최신 버전·테스트 수)은 여기 적지 않는다** — 목록의 정본은 `backend/settings.gradle.kts`, 스키마는 `backend/app/src/main/resources/db/migration/` 이다 (손 카운트는 드리프트한다 — `frontend/docs/structure.md` §유지 규약의 같은 규칙). Frontend is scaffolded and under active feature work — architecture canon is `frontend/README.md` (TRIP-160), FSD 층 구조에 TS/TSX 소스와 Jest 테스트가 실재한다. `ai/` 도 **실서비스 코드가 있다** — FastAPI 경계 9종이 열려 있고(`/ai/v1/itinerary/{generate,validate,repair,alternatives,explanations,edit}` + `/ai/v1/reflection/{generate,nudge}` + `/ai/v1/notification/copies`), 그중 백엔드가 실제로 부르는 것은 4종이다(generate·validate·repair·explanations — `HttpScheduleAgentAdapter`). Most design content is Korean documents — do not invent build/test/lint results.
...
| `ai/` | AI 서비스(FastAPI) + 설계 문서. `ai/src/trippilot/` 에 api·agents·orchestrator·assembly_engine·llm_gateway 가 실재한다. **와이어 정본은 `ai/docs/openapi.json`** — AI CI 가 "실행 앱 스키마 == 커밋된 계약"을 강제하므로 항상 실서버와 일치한다. |
...
- **INV-2** User-visible times/order come only from assembly-verified values (routers/workers only *propose*).
...
- **AI/Assembly (`ai/`):** Python service (FastAPI + LLM gateway + assembly). 경계 9종이 열려 있고(위 §What this is), 백엔드는 그중 4종을 부른다. **슬롯 후보는 AI 경로가 없다** — 백엔드가 로컬 후보풀로 폴백하며 `degraded=true` 로 표시한다(`HttpScheduleAgentAdapter.proposeSlotCandidates`). 백엔드는 기본 `AI_SCHEDULE_MODE=fake` 로 돌고, 실 연동은 `AI_SCHEDULE_MODE=http` + `docker compose --profile full up -d ai`. 왕복 검증은 `LIVE_AI=1 ./gradlew :app:test --tests "*LiveAiRoundTripIT*"`.
- **CI:** 머지 게이트 3종 `.github/workflows/{backend-ci, frontend-ci, ai-ci}.yml` (path-filtered) + 실 API 를 쓰는 스케줄 배치 3종 `{ai-poi-collect, ai-event-collect, ai-llm-smoke}.yml` (cron·수동 실행 전용, 경로 필터 없음). Gates before merge: assembly hard-constraint PBT at 100%, closed-set gate PBT at 100%, and **all external APIs (LLM, distance/maps) faked in CI — zero real API calls**.
